#!/usr/bin/env python3
"""
TimeGAN Stage 4 - WGAN-GP (Wasserstein GAN with Gradient Penalty)
Master Thesis - Generative AI Workload Modeling
Fachhochschule Dortmund

WHAT CHANGED FROM STAGE 3
==========================
Stage 3 result: mean VR = 0.408 vs S2 0.511 vs LSTM 0.612.
Stage 3 REGRESSED because moment matching (lambda_var=0.5) destabilized YOLO
(-0.49 delta) by adding an uncalibrated loss term that broke the adversarial
equilibrium Stage 2 had carefully established.

Stage 3 conclusion: adding more loss terms on top of BCE is fragile. The
correct fix is to replace the adversarial objective itself.

ROOT CAUSE OF ALL REMAINING FAILURES
=====================================
BCE adversarial loss has a fundamental problem: when the discriminator is even
slightly better than the generator, its gradients saturate (log(D(x)) -> 0).
The generator receives near-zero gradient signal. Spectral norm (Stage 2)
helped by constraining how fast the discriminator wins, but did not fix the
underlying gradient saturation.

WGAN-GP SOLUTION
================
Wasserstein GAN (Arjovsky et al. 2017) + Gradient Penalty (Gulrajani et al. 2017)
replaces the BCE adversarial loss with the Wasserstein-1 distance:

    W(P_real, P_fake) = max_{||f||_L <= 1} E[f(real)] - E[f(fake)]

The critic (discriminator without sigmoid) outputs an unbounded score. The
Lipschitz constraint ||f||_L <= 1 is enforced via gradient penalty:

    GP = E[ (||grad_x f(x_hat)||_2 - 1)^2 ]
    where x_hat = epsilon * real + (1-epsilon) * fake  (interpolated samples)

CHANGES FROM STAGE 3
=====================
1. Discriminator: remove sigmoid (output is now an unbounded Wasserstein score)
   Remove spectral norm (GP replaces it as the Lipschitz constraint)
   Keep get_features() for feature matching.

2. Critic loss:  L_C = mean(C(fake)) - mean(C(real)) + lambda_gp * GP
   Minimize this -> critic scores real high, fake low.

3. Generator adversarial loss: L_adv = -mean(C(fake))
   (maximize critic score for fakes, no saturation possible)

4. Keep feature matching at small weight (0.01) - theoretically sound,
   proven non-destabilizing in S3 (BERT improved +0.024 from it).
   Drop lambda_var and lambda_auto entirely - S3 proved they destabilize.

5. n_disc_steps=2: the 5-step recommendation is for pure WGAN without any
   reconstruction loss. With MSE already anchoring the generator, 2 critic
   steps per generator step is sufficient and keeps training fast.

6. Two-timescale LR kept: lr_gen=1e-3, lr_disc=2e-4.

Combined generator loss:
    L_G = lambda_rec * MSE(fake, target)
        + lambda_adv * (-mean(C(fake)))
        + lambda_fm  * MSE(mean(C_feat(fake)), mean(C_feat(real)).detach())

Critic loss:
    L_C = mean(C(fake).detach()) - mean(C(real))
        + lambda_gp * E[(||grad C(x_hat)||_2 - 1)^2]

ABLATION PLAN
=============
Stage 1: minimal GAN                                    -> VR 0.371
Stage 2: + spectral norm + 2-timescale LR + n_disc=2   -> VR 0.511
Stage 3: + feature matching + moment matching           -> VR 0.408 (regression)
Stage 4 (this file): replace BCE with WGAN-GP
...
USAGE
-----
    python timegan_s4.py
    python timegan_s4.py --workloads bert --epochs 100
    python timegan_s4.py --lambda-gp 5.0 --n-disc-steps 3
"""
...
OUTPUT_DIR   = Path("outputs/phase4/timegan_s4")
MODEL_DIR    = Path("models/phase4/timegan_s4")
...
TRAIN_CFG = {
    ...
    "epochs":       200,
    "lr_gen":       1e-3,
    "lr_disc":      2e-4,    # two-timescale LR kept from Stage 2
    "weight_decay": 1e-5,
    "patience":     25,
    "min_delta":    1e-5,
    "lambda_rec":   1.0,
    "lambda_adv":   0.1,     # weight on -mean(C(fake)) generator term
    "lambda_fm":    0.01,    # feature matching: small, non-destabilizing
    "lambda_gp":    10.0,    # gradient penalty coefficient (standard WGAN-GP value)
    "n_disc_steps": 2,       # 2 is sufficient: MSE reconstruction loss anchors the generator.
                             # The 5-step recommendation is for pure WGAN without recon loss.
    "clip_stds":    2.5,
    "n_gen":        5,
}
...
class Discriminator(nn.Module):
    """
    BiLSTM critic for WGAN-GP.

    WGAN-GP changes vs Stage 2/3 discriminator:
    - No sigmoid in classifier (output is unbounded Wasserstein score)
    - No spectral normalization (gradient penalty enforces Lipschitz constraint)

    The critic outputs a real-valued score, not a probability. Higher = more real.
    The Lipschitz constraint is enforced by penalizing gradient norms at
    interpolated points between real and fake distributions.

    get_features() is kept for feature matching.
    """
    def __init__(self, n_metrics, disc_cfg):
        ...
        # No spectral norm, no sigmoid — raw linear output is the Wasserstein score
    ...
    def forward(self, x):
        """x: (B, T, M) -> score: (B, 1)  (unbounded, not a probability)"""
        _, (h_n, _) = self.lstm(x)
        fwd = h_n[-2]
        bwd = h_n[-1]
    ...
    def get_features(self, x):
        """
        Pre-classifier hidden representation for feature matching.
        x: (B, T, M) -> features: (B, hidden_dim*2)
        """
        _, (h_n, _) = self.lstm(x)
        fwd = h_n[-2]
        bwd = h_n[-1]
...
def gradient_penalty(critic, real, fake, device):
    ...
    create_graph=True requires backprop through the LSTM forward pass (double
    backward). CuDNN's RNN kernel does not expose its internals to autograd,
    so this always fails with CuDNN enabled regardless of parameter freezing.
    The only correct fix is to disable CuDNN for this one forward call so
    PyTorch uses its own RNN implementation that supports double backward.

    This call is: 1 forward pass per critic update step, on a batch of B=16
    sequences. The rest of training (all generator and critic updates) stays
    on CuDNN at full speed.
...
def train_timegan(generator, discriminator, train_dl, val_dl,
    ...
    Critic loss  = mean(C(fake)) - mean(C(real)) + lambda_gp * GP
    Generator loss = lambda_rec  * MSE(fake, target)
                   + lambda_adv  * (-mean(C(fake)))
                   + lambda_fm   * MSE(mean(C_feat(fake)), mean(C_feat(real)).detach())

    Early stopping monitors generator reconstruction loss. Adversarial and
    Wasserstein losses oscillate by design and should not trigger stopping.

    WGAN-GP uses Adam with (beta1=0, beta2=0.9) as recommended in the paper.
    beta1=0 removes momentum on the first moment, which helps with the
    non-stationary loss landscape of adversarial training.
    """
    # WGAN-GP recommended optimizer settings: beta1=0, beta2=0.9
    opt_G = torch.optim.Adam(generator.parameters(),
                             lr=train_cfg["lr_gen"],
                             betas=(0.0, 0.9),
                             weight_decay=train_cfg["weight_decay"])
    opt_C = torch.optim.Adam(discriminator.parameters(),
    ...
    # Save initial weights as fallback so load_state_dict never receives None.
    best_state_G = {k: v.cpu().clone() for k, v in generator.state_dict().items()}
    best_state_C = {k: v.cpu().clone() for k, v in discriminator.state_dict().items()}
    patience_c   = 0
    ...
    for epoch in range(1, train_cfg["epochs"] + 1):
        ...
        epoch_loss_rec = []
        epoch_loss_adv = []
        epoch_w_dist   = []   # Wasserstein estimate: mean(C(real)) - mean(C(fake))
        ...
        for x_ph, target, _, r_norm, exp_ids, ph_batch in train_dl:
            ...
            # ---- Critic update(s) ----
            # Generator frozen during critic updates for efficiency
            for _ in range(train_cfg["n_disc_steps"]):
                with torch.no_grad():
                    fake = generator(x_ph, r_norm, exp_ids, ph_batch)  # (B, T, M)

                score_real = discriminator(target)      # (B, 1)
                score_fake = discriminator(fake)        # (B, 1)

                gp = gradient_penalty(discriminator, target.detach(), fake.detach(), device)

                # Critic maximizes E[C(real)] - E[C(fake)], equivalently minimizes:
                loss_C = score_fake.mean() - score_real.mean() + train_cfg["lambda_gp"] * gp
        ...
        if epoch % 20 == 0 or epoch == 1:
            print(f"    ep {epoch:>4}  val_rec={val_rec:.5f}  "
                  f"G={h['loss_G']:.4f}  C={h['loss_C']:.4f}  "
                  f"rec={h['loss_rec']:.4f}  W={h['w_dist']:.4f}")

        if not math.isnan(val_rec) and val_rec < best_val_rec - train_cfg["min_delta"]:
    ...
    discriminator.load_state_dict(best_state_C)
...
def plot_comparison(workload, real_orig, syn_orig,
    ...
    if history:
        ...
        crit_loss= [h["loss_C"]   for h in history]
        w_dist   = [h["w_dist"]   for h in history]
        ...
        loss_ax.plot(eps, crit_loss, label="critic loss", linewidth=1.0, color="#2ca02c")
        loss_ax.plot(eps, w_dist,    label="W-dist",      linewidth=1.0, color="#9467bd",
                     linestyle=":")
    ...
    fig.suptitle(
        f"{workload.upper()} - TimeGAN Stage 4 (WGAN-GP)\n"
    ...
    path = save_dir / f"{workload}_timegan_s4.png"
...
def main():
    ...
    parser.add_argument("--lambda-fm",    type=float,default=TRAIN_CFG["lambda_fm"])
    parser.add_argument("--lambda-gp",    type=float,default=TRAIN_CFG["lambda_gp"])
    parser.add_argument("--n-disc-steps", type=int,  default=TRAIN_CFG["n_disc_steps"])
    ...
    TRAIN_CFG["lambda_gp"]    = args.lambda_gp
    TRAIN_CFG["n_disc_steps"] = args.n_disc_steps
    ...
    for workload in args.workloads:
        ...
        cfg_out = {
            **GEN_CFG, **DISC_CFG,
            ...
            "stage": "s4_wgan_gp",
            "spectral_norm": False,   # replaced by gradient penalty
            "lr_gen": TRAIN_CFG["lr_gen"],
            "lr_disc": TRAIN_CFG["lr_disc"],
            "n_disc_steps": TRAIN_CFG["n_disc_steps"],
        }
        ...
        all_results.append({
            ...
            "s1_vr": {
                "bert": 0.191, "gpt2": 0.532, "resnet152": 0.108,
                "whisper": 0.754, "yolo": 0.271
            }.get(workload),
            "s2_vr": {
                "bert": 0.252, "gpt2": 0.552, "resnet152": 0.308,
                "whisper": 0.761, "yolo": 0.680
            }.get(workload),
            "s3_vr": {
                "bert": 0.276, "gpt2": 0.539, "resnet152": 0.288,
                "whisper": 0.746, "yolo": 0.190
            }.get(workload),
        })

    print(f"\n{'='*65}")
    print("TIMEGAN STAGE 4 SUMMARY")
    print(f"{'='*65}")
    header = f"{'Workload':<12}  {'VR S4':>8}  {'VR S2':>8}  {'VR S3':>8}  {'VR LSTM':>8}  {'Delta S2->S4':>13}"
    print(header)
    print("-" * len(header))
    for r in all_results:
        lstm_vr = r["lstm_baseline_vr"] or 0.0
        s2_vr   = r["s2_vr"] or 0.0
        s3_vr   = r["s3_vr"] or 0.0
        delta   = r["var_ratio_mean"] - s2_vr
    ...
    print()
    print("  S1 mean VR   = 0.371  (minimal GAN)")
    print("  S2 mean VR   = 0.511  (stabilization)")
    print("  S3 mean VR   = 0.408  (feature/moment matching - regressed)")
    print("  LSTM mean VR = 0.612  (baseline)")
    print()
    print("  If S4 > S2: WGAN-GP improved over best GAN result")
    print("  If S4 > LSTM: GAN now strictly better than LSTM baseline")
    print("  If S4 ~ S2: adversarial objective is not the bottleneck -> try S5 (conditional disc)")

    out_path = OUTPUT_DIR / "timegan_s4_results.json"
