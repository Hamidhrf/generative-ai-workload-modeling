#!/usr/bin/env python3
"""
TimeGAN Stage 2 - Stabilization
Master Thesis - Generative AI Workload Modeling
Fachhochschule Dortmund

WHAT CHANGED FROM STAGE 1
==========================
Stage 1 result: mean VR = 0.371 vs LSTM 0.612. GAN hurt all workloads.

Diagnosed failure from loss curves:
  - ResNet152: discriminator loss -> 0 at epoch 5, adversarial spikes to 7.5,
               collapse at epoch 27. Classic "discriminator wins instantly".
  - BERT/YOLO: discriminator oscillates 1.3-2.0, adversarial spikes unstable.
               Generator receiving noisy/saturated gradients.
  - GPT2:      More stable -> best VR of the group (0.532). Confirms that
               stability = performance.
  - Whisper:   Discriminator flat at 1.4, adversarial flat at 0.7. Most stable
               -> only workload that didn't regress from LSTM.

Three fixes added (all three target the same root cause: discriminator too strong):

1. SPECTRAL NORMALIZATION on all discriminator linear layers
   Constrains the Lipschitz constant of D, preventing it from producing
   saturated gradients that kill the generator signal.
   Applied via nn.utils.spectral_norm() wrapper — zero hyperparameters.

2. TWO-TIMESCALE LEARNING RATES: gen_lr=1e-3, disc_lr=2e-4
   Discriminator updates 5x slower than generator. This matches the
   empirical observation that the discriminator converges faster and needs
   to be intentionally slowed to maintain competitive equilibrium.

3. n_disc_steps=2 per generator step
   Compensates for the slower disc LR — discriminator still sees enough
   gradient to learn, just at a controlled pace.

Nothing else changed. Generator identical to Stage 1 (= LSTM v6).
Loss functions identical to Stage 1.

ABLATION PLAN
=============
Stage 1: minimal GAN                             -> VR 0.371
Stage 2 (this file): + spectral norm + 2-timescale LR + n_disc=2
...
USAGE
-----
    python timegan_s2.py
    python timegan_s2.py --workloads bert --epochs 50
"""
...
OUTPUT_DIR   = Path("outputs/phase4/timegan_s2")
MODEL_DIR    = Path("models/phase4/timegan_s2")
...
def plot_comparison(workload, real_orig, syn_orig,
    ...
    fig.suptitle(
        f"{workload.upper()} - TimeGAN Stage 2 (stabilization)\n"
    ...
    path = save_dir / f"{workload}_timegan_s2.png"
...
def main():
    ...
    print("=" * 65)
    print("TimeGAN Stage 2 - Stabilization")
    ...
    print("Stage 2 additions: spectral norm + two-timescale LR + n_disc=2")
    print("=" * 65)
    ...
    print(f"\n{'='*65}")
    print("TIMEGAN STAGE 2 SUMMARY")
    print(f"{'='*65}")
    header = f"{'Workload':<12}  {'VR S2':>8}  {'VR S1':>8}  {'VR LSTM':>8}  {'Delta S1->S2':>13}"
    print(header)
    print("-" * len(header))
    for r in all_results:
        lstm_vr = r["lstm_baseline_vr"] or 0.0
        s1_vr   = r["s1_vr"] or 0.0
        delta   = r["var_ratio_mean"] - s1_vr
    ...
    print()
    print("  S1 mean VR  = 0.371  (minimal GAN, no stabilization)")
    print("  LSTM mean VR = 0.612  (baseline)")
    print()
    print("  If S2 > S1: stabilization helped the adversarial signal")
    print("  If S2 > LSTM: adversarial training is now net positive")
    print("  If S2 ~ S1: discriminator still dominating despite fixes")

    out_path = OUTPUT_DIR / "timegan_s2_results.json"
