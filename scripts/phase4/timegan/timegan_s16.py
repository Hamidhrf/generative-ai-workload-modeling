#!/usr/bin/env python3
"""
TimeGAN S16 - Per-Phase Statistical Feature Matching
======================================================
...
S15 DIAGNOSIS
==============
S15 (mean VR=1.144) fixed the zeromean problem. ResNet152 is genuinely
excellent (all 5 metrics 1.18-1.26). Three workloads still have low-VR
metrics and one has discriminator collapse.

PROBLEM 1: THROUGHPUT FLAT ON BERT, GPT2, YOLO (VR 0.40-0.60)
  Global fm_stat computes:
      real_r.mean(dim=[0, 1])    # scalar per metric, averaged over 720 timesteps
  For throughput: phase 1 mean ~0.1, phase 3 mean ~4.5, global mean ~1.5.
  The generator satisfies fm_stat perfectly by outputting a constant ~1.5
  across all timesteps. The loss reaches zero. Generator has no reason to
  reproduce the phase-dependent step changes.

  Evidence:
    BERT  throughput: synthetic constant ~0.5-1.5, real steps from 0.4 to 4.2
    GPT2  latency:    synthetic flat at 0.75, real steps across phases
    YOLO  throughput: synthetic constant ~0.8, real spans 0.7 to 4.6

PROBLEM 2: WHISPER DISCRIMINATOR COLLAPSE (W=29 at end)
  n_disc_steps=3 causes discriminator to win too easily. The generator
  receives large adversarial loss signals that destabilise training.
  cpu_usage VR=0.43, latency VR=0.44.

S16 FIX 1: PER-PHASE FM_STAT
================================
Replace the single global fm_stat with per-phase fm_stat:
  For each phase p in {0..5}:
    For each unique replica count r in batch:
      real_rp = target[r_mask, phase_p_timesteps, :]
      fake_rp = fake[r_mask, phase_p_timesteps, :]
      loss += MSE(fake_rp.mean(dim=[0,1]), real_rp.mean(dim=[0,1]))
      loss += MSE(fake_rp.std(dim=[0,1]),  real_rp.std(dim=[0,1]))

This forces the generator to match correct throughput levels WITHIN each
phase:
  "In phase 1, throughput should be ~0.1. In phase 3, throughput ~4.5."
The generator can no longer collapse to the global mean.

Phase masks are precomputed once from phase_seq and passed to train().

S16 FIX 2: WHISPER N_DISC_STEPS 3 -> 2
=========================================
Reduces discriminator advantage on Whisper. At 3 steps the discriminator
dominated (W=29). At 2 steps it remains competitive without collapsing.

CHANGES FROM S15
==================
fm_stat computation:   global mean/std -> per-phase mean/std per replica count
Whisper n_disc_steps:  3 -> 2
run_tag:               s15 -> s16
All other code:        identical to S15
...
S16            THIS   per-phase fm_stat, Whisper disc fix

USAGE
-----
    python timegan_s16.py
    python timegan_s16.py --workloads bert yolo
    python timegan_s16.py --var-reg 0.3 --fm-stat 0.5 --adv-epochs 150

OUTPUT
------
    outputs/phase4/timegan_s16/{run_tag}/plots/
    models/phase4/timegan_s16/{run_tag}/{workload}/
    outputs/phase4/timegan_s16/{run_tag}/results.json

    run_tag = s16_vr{var_reg*10}_fm{fm_stat*10}_ae{adv_epochs}
"""
...
# S16: Whisper n_disc_steps reduced from 3 to 2
WORKLOAD_OVERRIDES = {
    "bert":      {"lambda_smooth": 0.1, "n_disc_steps": 2},
    "gpt2":      {"lambda_smooth": 0.1, "n_disc_steps": 2},
    "resnet152": {"lambda_smooth": 0.1, "n_disc_steps": 2},
    "whisper":   {"lambda_smooth": 0.3, "n_disc_steps": 2},
    "yolo":      {"lambda_smooth": 0.1, "n_disc_steps": 2},
}

REFERENCE_VR = {
    "bert":      {"lstm": 0.594, "s9": 1.213, "s15": 1.631},
    "gpt2":      {"lstm": 0.676, "s9": 0.664, "s15": 0.775},
    "resnet152": {"lstm": 0.607, "s9": 0.324, "s15": 1.207},
    "whisper":   {"lstm": 0.748, "s9": 0.743, "s15": 0.838},
    "yolo":      {"lstm": 0.438, "s9": 1.446, "s15": 1.270},
}

REFERENCE_JUMP = {
    "bert":      {"s8":  3.9, "s9":  8.7, "s15": 0.064},
    "gpt2":      {"s8":  3.0, "s9":  7.2, "s15": 0.136},
    "resnet152": {"s8":  6.5, "s9": 17.1, "s15": 0.036},
    "whisper":   {"s8": 42.1, "s9": 26.2, "s15": 0.396},
    "yolo":      {"s8": 10.6, "s9": 11.2, "s15": 0.192},
}
...
def train(generator, discriminator, train_dl, val_dl,
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for target, r_int, r_norm, ph_batch in train_dl:
            ...
            # S16 KEY CHANGE: replaces global fm_stat with per-phase fm_stat
            if lambda_fm_stat > 0:
...
def main():
    parser = argparse.ArgumentParser(description="TimeGAN S16 - Per-Phase FM Stat")
    ...
    run_tag    = (f"s16_vr{int(args.var_reg * 10):02d}"
                  f"_fm{int(args.fm_stat   * 10):02d}"
                  f"_ae{args.adv_epochs}{interp_tag}")

    out_dir   = Path(f"outputs/phase4/timegan_s16/{run_tag}")
    model_dir = Path(f"models/phase4/timegan_s16/{run_tag}")
    out_dir.mkdir(parents=True, exist_ok=True)
    model_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 70)
    print("TimeGAN S16 - Per-Phase Statistical Feature Matching")
    print("=" * 70)
    ...
    print(f"Per-phase FM    : lambda_fm_stat={args.fm_stat}  (S16: per-phase, not global)")
    ...
    for workload in args.workloads:
        ...
        print(f"    var_ratio     = {vr_mean:.4f}  "
              f"(s15={ref_vr.get('s15','?'):.4f}  s9={ref_vr.get('s9','?'):.4f}  "
              f"lstm={ref_vr.get('lstm','?'):.4f})")
        print(f"    autocorr_diff = {ac_diff:.4f}")
        print(f"    jump_ratio    = {jump_ratio:.3f}x  "
              f"(s15={ref_jump.get('s15','?'):.3f}x  s9={ref_jump.get('s9','?'):.1f}x)")
