#!/usr/bin/env python3
"""
TimeGAN S18 - Per-Workload FM/VarReg Tuning + Whisper Stat Checkpoint
=======================================================================
...
S17 RESULTS
============
S17 (mean VR=1.003) fixed YOLO disc explosion and improved Whisper from 0.543
to 0.636. ResNet152 is now solved (all per-metric VR ~1.02). GPT2 improved to
1.011. However three per-metric issues remain.

PROBLEM 1: BERT cpu VR=0.651, YOLO throughput VR=0.650
  The adversarial loss G grows to 4-5 by late training while fm_stat stays at
  ~0.054 — roughly a 100:1 ratio. The per-phase anchor cannot compete with the
  adversarial gradient when W-dist diverges this strongly. Increasing
  lambda_fm_stat for bert/yolo will improve the ratio and anchor the level.
  Reducing lambda_smooth to 0.05 gives more temporal freedom.

PROBLEM 2: WHISPER VR=0.636, W-dist stuck near 0.007
  Whisper's adversarial training never meaningfully engages: G stays negative
  throughout (G~=-0.25), W oscillates near 0. The best_W checkpoint therefore
  selects from near-untrained weights (W=0.0071). The best_val_stat checkpoint
  at 0.0197 represents the best statistical fit the model achieved. Using
  best_stat selection for Whisper recovers the warmup-trained model instead of
  discarding it. Additionally lambda_fm_stat=1.0 over-constrains Whisper (with
  G negative, fm_stat is the dominant loss, compressing variance). Reducing
  fm_stat to 0.5 and increasing var_reg to 0.5 gives variance more room.

PROBLEM 3: GPT2 gpu_memory VR=0.573
  gpu_memory in GPT2 shows very tight bands in real data (near-static
  allocation). The generator introduces slightly more variance than real.
  This is a data characteristic, not a training issue. Accepted as-is and
  documented for thesis.

S18 FIX 1: BERT + YOLO lambda_fm_stat 1.0->1.5, lambda_smooth 0.1->0.05
===========================================================================
Increases the fm_stat-to-adversarial ratio for bert and yolo, anchoring
cpu and throughput level targets more firmly. Reducing lambda_smooth removes
the temporal suppression that contributes to under-generation.

S18 FIX 2: WHISPER checkpoint_by="best_stat"
=============================================
Add per-workload checkpoint_by override. When set to "best_stat", the training
loop loads best_G_stat (best warmup/val checkpoint) at the end instead of
best_G (best W-dist). For Whisper, W-dist never diverges so best_W selects a
near-untrained model. best_stat selects the statistically best checkpoint.

S18 FIX 3: WHISPER lambda_var_reg 0.3->0.5, lambda_fm_stat 1.0->0.5
======================================================================
With W~0 and G negative, fm_stat=1.0 dominates and compresses synthetic
variance. Reducing fm_stat to 0.5 reduces over-anchoring. Increasing var_reg
to 0.5 gives the variance penalty more weight relative to fm_stat.
The combination: more variance freedom + better checkpoint selection.

CHANGES FROM S17
==================
WORKLOAD_OVERRIDES:
  bert:      lambda_fm_stat 1.0->1.5, lambda_smooth 0.1->0.05
  whisper:   lambda_fm_stat 1.0->0.5, lambda_var_reg 0.3->0.5,
             checkpoint_by "best_W"->"best_stat"
  yolo:      lambda_fm_stat 1.0->1.5, lambda_smooth 0.1->0.05
  gpt2:      unchanged (VR=1.011, gpu_memory accepted as data characteristic)
  resnet152: unchanged (all per-metric VR ~1.02, solved)
train():     new checkpoint_by parameter ("best_W" default, "best_stat" override)
main():      pass checkpoint_by from WORKLOAD_OVERRIDES to train()
run_tag:     s17->s18
references:  s17 values added
...
S18            THIS   per-workload fm/var tuning; Whisper best_stat checkpoint

USAGE
-----
    python timegan_s18.py
    python timegan_s18.py --workloads bert yolo whisper
    python timegan_s18.py --var-reg 0.3 --fm-stat 1.0 --adv-epochs 150

OUTPUT
------
    outputs/phase4/timegan_s18/{run_tag}/plots/
    models/phase4/timegan_s18/{run_tag}/{workload}/
    outputs/phase4/timegan_s18/{run_tag}/results.json

    run_tag = s18_vr{var_reg*10}_fm{fm_stat*10}_ae{adv_epochs}
"""
...
# S18: per-workload fm_stat/var_reg/checkpoint_by overrides
# bert/yolo: lambda_fm_stat 1.0->1.5, lambda_smooth 0.1->0.05
# whisper:   lambda_fm_stat 1.0->0.5, lambda_var_reg 0.3->0.5,
#            checkpoint_by "best_W"->"best_stat"
# gpt2/resnet152: unchanged from S17
WORKLOAD_OVERRIDES = {
    ...
    "whisper":   {"lambda_smooth": 0.05, "n_disc_steps": 1,
                  ...
                  "checkpoint_by": "best_stat"},
    "yolo":      {"lambda_smooth": 0.05, "n_disc_steps": 2,
                  "lambda_fm_stat": 1.5,  "lambda_var_reg": 0.3,
                  "checkpoint_by": "best_W"},
}

REFERENCE_VR = {
    "bert":      {"lstm": 0.594, "s9": 1.213, "s17": 1.116},
    "gpt2":      {"lstm": 0.676, "s9": 0.664, "s17": 1.011},
    "resnet152": {"lstm": 0.607, "s9": 0.324, "s17": 1.024},
    "whisper":   {"lstm": 0.748, "s9": 0.743, "s17": 0.636},
    "yolo":      {"lstm": 0.438, "s9": 1.446, "s17": 1.228},
}

REFERENCE_JUMP = {
    "bert":      {"s8":  3.9, "s9":  8.7, "s17": 0.051},
    "gpt2":      {"s8":  3.0, "s9":  7.2, "s17": 0.158},
    "resnet152": {"s8":  6.5, "s9": 17.1, "s17": 0.095},
    "whisper":   {"s8": 42.1, "s9": 26.2, "s17": 0.367},
    "yolo":      {"s8": 10.6, "s9": 11.2, "s17": 0.122},
}
...
def main():
    ...
    out_dir   = Path(f"outputs/phase4/timegan_s18/{run_tag}")
    model_dir = Path(f"models/phase4/timegan_s18/{run_tag}")
    ...
    for workload in args.workloads:
        ...
        print(f"    var_ratio     = {vr_mean:.4f}  "
              f"(s17={ref_vr.get('s17','?'):.4f}  s9={ref_vr.get('s9','?'):.4f}  "
              f"lstm={ref_vr.get('lstm','?'):.4f})")
        print(f"    autocorr_diff = {ac_diff:.4f}")
        print(f"    jump_ratio    = {jump_ratio:.3f}x  "
              f"(s17={ref_jump.get('s17','?'):.3f}x  s9={ref_jump.get('s9','?'):.1f}x)")
