"""
TimeGAN S24 - Segment-Based Generation + Autocorrelation Loss
========================================
Master Thesis - Generative AI Workload Modeling
Fachhochschule Dortmund

S24 vs S21: ONE CHANGE ONLY
==============================
Added autocorrelation loss term to generator update (Fix B).

  autocorr_loss(real_seg, fake_seg, max_lag=20)

This loss penalises the difference in mean autocorrelation structure
between real and synthetic segments at each lag 1..max_lag.
It runs during BOTH warmup and adversarial phases (same as var_reg and
fm_stat), giving the generator a temporal structure signal from epoch 1.

S21 RESULTS (starting point)
==============================
  bert      VR=0.672
  gpt2      VR=0.996
  resnet152 VR=0.960
  whisper   VR=1.284
  yolo      VR=0.661
  MEAN      0.9146  > 0.8 target achieved

S21 IDENTIFIED FAILURE MODES
==============================
1. Temporal smoothing (all workloads): synthetic autocorrelation decays
   faster than real. Mean |diff|: BERT=0.152, GPT2=0.118, ResNet152=0.140,
   Whisper=0.149, YOLO=0.272. Root cause: LSTM generators produce smoother
   traces with no training incentive to preserve autocorrelation structure.

2. Whisper replica ordering inversion: real CPU non-monotonic (peaks r=2,
   falls at r=10 due to CPU starvation). Model learned monotonic mapping.
   Real physics -- not fixed in S24 (requires architectural change).

3. GPT2 gpu_memory_used VR=0.06: pre-allocated memory, not dynamic.
   Defensible for thesis. Not targeted in S24.

S24 TARGETS
============
Fix temporal smoothing (failure mode 1) by adding autocorr_loss.
Starting lambda_ac=0.1. If BERT/YOLO VR improves without degrading
GPT2/Whisper/ResNet152, S24 becomes the submitted result.
Otherwise S21 remains.

USAGE
-----
    python timegan_s24.py
    python timegan_s24.py --workloads bert yolo
    python timegan_s24.py --lambda-ac 0.3
    python timegan_s24.py --var-reg 0.3 --fm-stat 1.0 --adv-epochs 150

OUTPUT
------
    outputs/phase4/timegan_s24/{run_tag}/plots/
    models/phase4/timegan_s24/{run_tag}/{workload}/
    outputs/phase4/timegan_s24/{run_tag}/results.json

    run_tag = s24_seg_vr{var_reg*10}_fm{fm_stat*10}_ae{adv_epochs}_ac{ac*10}
"""
...
# S24: same per-workload overrides as S21
WORKLOAD_OVERRIDES = {
...
REFERENCE_VR = {
    "bert":      {"lstm": 0.594, "s19": 1.226, "s21": 0.672},
    "gpt2":      {"lstm": 0.676, "s19": 1.011, "s21": 0.996},
    "resnet152": {"lstm": 0.607, "s19": 1.024, "s21": 0.960},
    "whisper":   {"lstm": 0.748, "s19": 0.788, "s21": 1.284},
    "yolo":      {"lstm": 0.438, "s19": 1.228, "s21": 0.661},
}
...
def autocorr_loss(real_seg, fake_seg, max_lag=20):
    ...
    for lag in range(1, max_lag + 1):
        ac_real = (real_d[:, :-lag, :] * real_d[:, lag:, :]).mean()
        ac_fake = (fake_seg[:, :-lag, :] * fake_seg[:, lag:, :]).mean()
...
def train(generator, discriminator, train_dl, val_dl,
    ...
    Identical to S21 except:
      loss_G += lambda_ac * autocorr_loss(target, fake, ac_max_lag)

    Active during BOTH warmup and adversarial phases (same as var_reg
    and fm_stat). Set lambda_ac=0 to reproduce exact S21 behaviour.
    """
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for target, r_int, r_norm, phase_idx in train_dl:
            ...
            # S24 NEW: Autocorrelation loss (warmup + adversarial)
            if lambda_ac > 0:
...
def main():
    parser = argparse.ArgumentParser(description="TimeGAN S24 segment-based + autocorr loss")
    ...
    run_tag   = (f"s24_seg_vr{int(args.var_reg * 10):02d}"
                 f"_fm{int(args.fm_stat * 10):02d}"
                 f"_ae{args.adv_epochs}"
                 f"_ac{int(args.lambda_ac * 10):02d}")
    out_dir   = Path(f"outputs/phase4/timegan_s24/{run_tag}")
    model_dir = Path(f"models/phase4/timegan_s24/{run_tag}")
    out_dir.mkdir(parents=True, exist_ok=True)

    boundaries = DEFAULT_PHASE_BOUNDARIES

    print("=" * 70)
    print("TimeGAN S24 - Segment-Based Generation + Autocorrelation Loss")
    print("=" * 70)
    ...
    for workload in args.workloads:
        ...
        print(f"    var_ratio     = {vr_mean:.4f}  "
              f"(s21={ref_vr.get('s21', '?')}  s19={ref_vr.get('s19', '?')}  "
    ...
    print(f"\n{'=' * 72}")
    print(f"S24 SUMMARY  ({run_tag})")
    print(f"{'=' * 72}")
    print(f"  {'Workload':<12} {'VR S24':>8} {'VR S21':>8} {'VR S19':>8} "
          f"{'LSTM':>8} {'jump S24':>10}")
    print(f"  {'-' * 72}")
    vr_means = []
    ...
    print(f"S21 baseline: mean VR=0.9146")
