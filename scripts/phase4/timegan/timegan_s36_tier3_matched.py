...
timegan_s36_tier3.py -- only DATA_COMBINED, NORM_COMBINED, and the two
output path prefixes (models/phase4/timegan_s36_tier3_matched/,
outputs/phase4/timegan_s36_tier3_matched/) changed, pointing at the
r=1..7-restricted tier3_matched dataset instead of full Tier 3.
S27_HYPERPARAMS, architecture, seeds, epoch counts, r_norm convention
((r-1)/9.0, unchanged even though this dataset only spans r=1..7 --
same convention Tier 2 keeps) are byte-identical to timegan_s36_tier3.py.
This is the frozen recipe on a restricted dataset, not a new tuning.
...
def main():
    ...
    for wl in args.workloads:
        ...
        model_dir = Path(f"models/phase4/timegan_s36_tier3_matched/{tag}")
