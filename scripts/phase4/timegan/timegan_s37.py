"""
TimeGAN S37 - S36 + Discriminator Dropout
==========================================
Master Thesis - Generative AI Workload Modeling
Fachhochschule Dortmund

S37 = S36 Architecture + Discriminator Dropout
===============================================

ABLATION STUDY: Does discriminator dropout improve VR?
- S36 baseline: Mean VR = 1.116 (spectral normalization)
- S37: Same architecture + dropout in discriminator LSTM

DISCRIMINATOR DROPOUT:
- Add dropout (p=0.3) to discriminator LSTM layers
- Prevents overconfident discriminator predictions
- Similar goal to label smoothing (but works with WGAN-GP)
- Improves generalization, reduces overfitting

RATIONALE:
- S36 improved overall (0.205→0.116 distance from VR=1.0)
- But BERT/GPT2 slightly degraded (+0.022, +0.032)
- Discriminator dropout should help these stable workloads
- Expected: Better balance across all workloads

USAGE:
------
    python timegan_s37.py --workloads bert gpt2 resnet152 whisper yolo
    python timegan_s37.py --workloads bert  # Train only BERT

OUTPUT:
-------
    models/phase4/timegan_s37/s37_{workload}_vr{X}_fm{Y}/
    outputs/phase4/timegan_s37/s37_summary.json
"""
...
DISC_CFG = {
    "hidden_dim": 32,
    "num_layers": 2,  # S37: Increased from 1 to enable dropout
    "dropout": 0.3,   # S37: NEW! Discriminator dropout for regularization
}
...
class DiscriminatorSpectral(nn.Module):
    """S37 discriminator: S36 (spectral norm) + dropout in LSTM"""
    def __init__(self, n_metrics, cfg):
        ...
        # S37: Now uses 2 layers with dropout=0.3
...
def train_workload(workload_name, hyperparams, device):
    """Train S37 model for single workload."""
    print(f"\n{'='*80}")
    print(f"S37 TRAINING: {workload_name.upper()}")
    print(f"{'='*80}")
    ...
    # Models - S37 uses spectral norm + dropout discriminator
    ...
    print(f"Discriminator: Spectral Norm + Dropout (p=0.3, 2 layers)")
...
def main():
    parser = argparse.ArgumentParser(description="S37 - S36 + Discriminator Dropout")
    ...
    print("="*80)
    print("S37: S36 + DISCRIMINATOR DROPOUT")
    print("="*80)
    print(f"Device: {device}")
    print(f"Workloads: {args.workloads}")
    print(f"Architecture: S36 + Dropout (p=0.3) in discriminator LSTM")
    print(f"Dataset: Original 176 traces (NO augmentation)")
    print(f"Improvement: Discriminator regularization via dropout")
    print("="*80)
    ...
    for wl in args.workloads:
        ...
        # Save model
        tag = f"s37_{wl}_vr{int(hyperparams['lambda_var_reg']*10):02d}_fm{int(hyperparams['lambda_fm_stat']*10):02d}"
        model_dir = Path(f"models/phase4/timegan_s37/{tag}")
    ...
    with open(out_dir / "s37_summary.json", "w") as f:
        json.dump({
            "stage": "s37",
            "description": "S36 architecture + Discriminator Dropout (p=0.3)",
    ...
    print("\n"+"="*80)
    print("S37 TRAINING COMPLETE")
    print("="*80)
    print(f"Summary: {out_dir / 's37_summary.json'}")
    print("\nNext: Evaluate with eval_s37.py to compare vs S36")
