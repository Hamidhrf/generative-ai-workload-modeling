"""
TimeGAN S22 - Segment-Based Generation Tuned
========================================
...
S22: SEGMENT-BASED GENERATION TUNED
================================
...
S22            THIS   segment-based generation tuned

USAGE
-----
    python timegan_s22.py
    python timegan_s22.py --workloads whisper yolo
    python timegan_s22.py --var-reg 0.3 --fm-stat 1.0 --adv-epochs 150

OUTPUT
------
    outputs/phase4/timegan_s22/{run_tag}/plots/
    models/phase4/timegan_s22/{run_tag}/{workload}/
    outputs/phase4/timegan_s22/{run_tag}/results.json

    run_tag = s22_seg_vr{var_reg*10}_fm{fm_stat*10}_ae{adv_epochs}
"""
...
WORKLOAD_OVERRIDES = {
    # S22 tuning rationale:
    # BERT:      vr=0.5 suppressed psi_cpu/gpu_util -> reduce to 0.3; fm up to 2.0 to anchor those metrics
    # GPT2:      gpu_util=3.0 (over) + gpu_mem=0.05 (under) -> smooth up to 0.15; fm up to 2.5
    # ResNet152: VR=0.99, jump=0.345x (marginal) -> small smooth reduction to 0.08 to allow sharper jumps
    # Whisper:   all metrics over-generating (VR=1.72) -> smooth up to 0.1; var_reg up to 0.65
    # YOLO:      all metrics uniformly low (0.53-0.77) -> smooth down to 0.05; var_reg down to 0.2
    "bert":      {"lambda_smooth": 0.05, "n_disc_steps": 2,
                  "lambda_fm_stat": 2.0,  "lambda_var_reg": 0.3,
                  "checkpoint_by": "best_W"},
    "gpt2":      {"lambda_smooth": 0.15, "n_disc_steps": 2,
                  "lambda_fm_stat": 2.5,  "lambda_var_reg": 0.3,
                  "checkpoint_by": "best_W"},
    "resnet152": {"lambda_smooth": 0.08, "n_disc_steps": 2,
                  "lambda_fm_stat": 1.0,  "lambda_var_reg": 0.4,
                  "checkpoint_by": "best_W"},
    "whisper":   {"lambda_smooth": 0.1,  "n_disc_steps": 1,
                  "lambda_fm_stat": 0.5,  "lambda_var_reg": 0.65,
                  "checkpoint_by": "best_W"},
    "yolo":      {"lambda_smooth": 0.05, "n_disc_steps": 2,
                  "lambda_fm_stat": 1.2,  "lambda_var_reg": 0.2,
                  "checkpoint_by": "best_W"},
}

REFERENCE_VR = {
    "bert":      {"lstm": 0.594, "s9": 1.213, "s19": 1.226, "s20": 0.621, "s21": 0.653},
    "gpt2":      {"lstm": 0.676, "s9": 0.664, "s19": 1.011, "s20": 1.253, "s21": 1.015},
    "resnet152": {"lstm": 0.607, "s9": 0.324, "s19": 1.024, "s20": 0.752, "s21": 0.990},
    "whisper":   {"lstm": 0.748, "s9": 0.743, "s19": 0.788, "s20": 0.661, "s21": 1.722},
    "yolo":      {"lstm": 0.438, "s9": 1.446, "s19": 1.228, "s20": 0.737, "s21": 0.630},
}

REFERENCE_JUMP = {
    "bert":      {"s8":  3.9, "s9":  8.7, "s19": 0.083, "s20": 2.104, "s21": 1.540},
    "gpt2":      {"s8":  3.0, "s9":  7.2, "s19": 0.158, "s20": 0.128, "s21": 3.199},
    "resnet152": {"s8":  6.5, "s9": 17.1, "s19": 0.095, "s20": 0.697, "s21": 0.345},
    "whisper":   {"s8": 42.1, "s9": 26.2, "s19": 0.285, "s20": 2.068, "s21": 2.793},
    "yolo":      {"s8": 10.6, "s9": 11.2, "s19": 0.122, "s20": 6.617, "s21": 1.941},
}
...
class GeneratorSeg(nn.Module):
    """
    S22 generator: produces one phase segment of SEGMENT_LEN=120 steps.
    ...
    720-step trace; in S22, phase_idx is constant throughout a segment.
    """
...
# ------------------------------------------------------------------
# S22 fm_stat: precomputed population-level targets + scale-invariant loss
# ------------------------------------------------------------------
...
def precompute_fm_targets(segments, seg_rc, seg_phase_idx, device):
    ...
    gradients -> VR degradation. S22 precomputes on full training set.
...
def compute_segment_fm_stat(fake, r_int, phase_idx_batch, fm_targets, device):
    """
    S22 fm_stat using precomputed population-level targets.
...
def train(generator, discriminator, train_dl, val_dl,
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for target, r_int, r_norm, phase_idx in train_dl:
            ...
            # S22: uses precomputed fm targets (scale-invariant)
            if lambda_fm_stat > 0:
...
def evaluate(generator, raw_kept, replica_counts, phase_seq,
    ...
    """
    Evaluation for S22 segment-based model.
...
# ------------------------------------------------------------------
# Visualisation (adapted for S22 -- plot full 720-step traces)
# ------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(description="TimeGAN S22 segment-based")
    ...
    run_tag  = (f"s22_seg_vr{int(args.var_reg * 10):02d}"
                f"_fm{int(args.fm_stat * 10):02d}"
                f"_ae{args.adv_epochs}")
    out_dir  = Path(f"outputs/phase4/timegan_s22/{run_tag}")
    model_dir = Path(f"models/phase4/timegan_s22/{run_tag}")
    ...
    print("=" * 70)
    print("TimeGAN S22 - Segment-Based Generation Tuned")
    print("=" * 70)
    ...
    for workload in args.workloads:
        ...
        # ------- S22 CORE: segment the full traces --------
        (all_segs, all_seg_rc,
