#!/usr/bin/env python3
"""
TimeGAN S11 - Conditional Discriminator + Variance Regression
=============================================================
Master Thesis - Generative AI Workload Modeling
Fachhochschule Dortmund

S10 RESULTS RECAP (run: s10_vp05_bc35_ae150)
==============================================
Mean VR 1.937, but inflated by psi_cpu hitting evaluation cap of 5.0
on BERT, GPT2, ResNet152, YOLO. Honest performance on remaining metrics
is ~0.6-1.0. Two structural problems identified:

PROBLEM 1: psi_cpu over-generation
  Real psi_cpu has near-zero variance. Adaptive cap set to ~21.0 for
  this metric (permissive by design). Generator produces arbitrary noise
  on psi_cpu with no effective penalty. Discriminator cannot penalise
  because it is unconditional.

PROBLEM 2: Discriminator is unconditional (critical bug)
  In S10 discriminator forward(): r_norm parameter accepted but never
  used. Generator conditions on replica count but discriminator cannot
  tell "your r=3 throughput looks like r=1 throughput". This is why
  throughput and gpu_utilization are replica-blind in synthetic traces
  and remain under-generated (VR 0.25-0.45).

S11 CHANGES (two targeted fixes)
==================================

CHANGE 1: Conditional Discriminator
  LSTM still processes raw sequence (replica-agnostic temporal patterns).
  Classifier receives LSTM hidden state concatenated with r_norm scalar.
  This allows the discriminator to penalise replica-blind generation.

  Discriminator classifier input changes from hidden*2 to hidden*2+1.
  gradient_penalty passes r_norm through discriminator.
  All discriminator calls in training loop pass r_norm.
  get_features keeps LSTM-only features (feature matching stays
  replica-agnostic to avoid overfitting to the conditioning signal).

CHANGE 2: Variance Regression replaces entire adaptive cap system
  Remove: compute_adaptive_cap(), cap_per_metric_tensor, lambda_var_pen,
          base_cap arg, cap_per_metric from config/results.
  Add:    Huber regression pulling fake std toward real std per metric.

      std_real = target.std(dim=[0, 1]).detach()   # (M,) target
      std_fake = fake.std(dim=[0, 1])               # (M,) differentiable
      var_reg  = F.huber_loss(std_fake, std_real, delta=0.5)
      loss_G   = loss_G + lambda_var_reg * var_reg

  delta=0.5: L2 for small errors, L1 for large errors.
  Pulls psi_cpu DOWN toward near-zero real std.
  Pulls throughput and gpu_util UP toward correct real std.
  No cap needed, no per-metric bookkeeping.
  Single backward pass (CuDNN RNNs do not support double backward).

ARCHITECTURE UNCHANGED FROM S8/S9/S10
========================================
Generator:     GeneratorA, LSTM h=128 l=2, enc-dropout per workload
Discriminator: Bidirectional LSTM h=32 l=1 (S7 capacity)
               S11: classifier input hidden*2+1 for r_norm injection
...
PER-WORKLOAD SETTINGS (carried from S10)
==========================================
...
EXPECTED OUTCOMES
==================
psi_cpu VR:          drops from 5.0 (capped) to 0.5-1.5 (correctly matched)
throughput/gpu_util: rises from 0.25-0.45 toward 0.5-1.2
Mean VR:             drops from inflated 1.937 to ~0.9-1.3 but honest
Whisper:             minimal change (already most balanced in S10)
...
S10  mean VR = 1.937  (inflated by psi_cpu capping)
LSTM mean VR = 0.612

USAGE
-----
    python timegan_s11.py                          # all workloads, defaults
    python timegan_s11.py --workloads bert gpt2    # subset
    python timegan_s11.py --var-reg 0.5            # variance regression strength
    python timegan_s11.py --smooth 0.0             # disable smoothing
    python timegan_s11.py --enc-dropout 0.3        # override all workloads

OUTPUT
------
    outputs/phase4/timegan_s11/{run_tag}/plots/
    models/phase4/timegan_s11/{run_tag}/{workload}/
    outputs/phase4/timegan_s11/{run_tag}/results.json

    run_tag = s11_vr{var_reg*10}_ae{adv_epochs}
    e.g.      s11_vr05_ae150
"""
...
# Per-workload overrides carried from S10
WORKLOAD_OVERRIDES = {
    "bert":      {"enc_dropout": 0.3,  "lambda_smooth": 0.1, "n_disc_steps": 2},
    "gpt2":      {"enc_dropout": 0.3,  "lambda_smooth": 0.1, "n_disc_steps": 2},
    "resnet152": {"enc_dropout": 0.3,  "lambda_smooth": 0.1, "n_disc_steps": 2},
...
REFERENCE_VR = {
    "bert":      {"lstm": 0.594, "s7": 0.240, "s8": 2.645, "s9": 1.213, "s10": 2.569},
    "gpt2":      {"lstm": 0.676, "s7": 0.573, "s8": 2.315, "s9": 0.664, "s10": 2.252},
    "resnet152": {"lstm": 0.607, "s7": 0.246, "s8": 1.055, "s9": 0.324, "s10": 2.017},
    "whisper":   {"lstm": 0.748, "s7": 0.745, "s8": 1.042, "s9": 0.743, "s10": 1.401},
    "yolo":      {"lstm": 0.438, "s7": 0.270, "s8": 1.334, "s9": 1.446, "s10": 1.446},
}

REFERENCE_JUMP = {
    "bert":      {"s7": 12.7, "s8":  3.9, "s9":  8.7, "s10": 0.33},
    "gpt2":      {"s7": 10.5, "s8":  3.0, "s9":  7.2, "s10": 1.98},
    "resnet152": {"s7": 12.6, "s8":  6.5, "s9": 17.1, "s10": 0.22},
    "whisper":   {"s7": 16.2, "s8": 42.1, "s9": 26.2, "s10": 0.89},
    "yolo":      {"s7": 10.2, "s8": 10.6, "s9": 11.2, "s10": 0.47},
}
...
# ------------------------------------------------------------------
# Replica interpolation (fixed skip condition from S9, carried in S10)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Generator (unchanged from S8/S9/S10)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Discriminator (S11: conditional on r_norm at classifier input)
# ------------------------------------------------------------------

class Discriminator(nn.Module):
    """
    Bidirectional LSTM followed by a conditional classifier.

    S11 change: r_norm is concatenated to the LSTM hidden state before
    the classifier MLP. The LSTM itself remains unconditional so that
    temporal feature extraction is replica-agnostic; conditioning happens
    only at the final classification step.

    Classifier input: hidden*2 + 1  (hidden*2 from biLSTM, +1 for r_norm)

    get_features returns LSTM-only features (hidden*2) without r_norm
    so that feature matching in the generator loss remains replica-agnostic
    and does not overfit to the conditioning signal.
    """
    def __init__(self, n_metrics, cfg):
        ...
        # +1 for r_norm injected at classifier input
    ...
    def forward(self, x, r_norm=None):
        ...
        if r_norm is not None:
            h = torch.cat([h, r_norm.unsqueeze(1)], dim=1)  # (B, hidden*2+1)
        else:
            # Fallback: zero-pad r_norm dimension if not supplied
            h = torch.cat([h, torch.zeros(h.shape[0], 1, device=h.device)], dim=1)
        return self.classifier(h)

    def get_features(self, x, r_norm=None):
        # Returns LSTM-only features without r_norm conditioning.
        # Feature matching uses these to stay replica-agnostic.
        _, (h_n, _) = self.lstm(x)
...
def gradient_penalty(disc, real, fake, r_norm, device, lam=10.0):
    """
    WGAN-GP gradient penalty with r_norm passed through discriminator.
    CuDNN RNNs do not support double backward; use non-CuDNN path.
    """
    B   = real.shape[0]
...
def train(generator, discriminator, train_dl, val_dl,
    ...
    1. All discriminator calls pass r_norm (conditional discriminator).
    2. gradient_penalty receives r_norm.
    3. Variance penalty replaced by Huber regression on per-metric std.
       - No cap, no per-metric bookkeeping.
       - Single backward pass throughout.
    4. Per-workload n_disc_steps passed in as n_disc_steps_wl.
    """
    ...
    print(f"    Adv:    {adv_epochs} fixed epochs  "
          f"var_reg={lambda_var_reg}  n_disc={n_disc_steps_wl}")
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        generator.train()
        discriminator.train()
        ep_rec = ep_adv_g = ep_loss_d = ep_wdist = ep_vreg = ep_sm = 0.0
        n_batches = 0
        ...
        for inp, target, _, r_norm, exp_ids, ph_batch in train_dl:
            ...
            # Discriminator update (passes r_norm)
            if n_disc_steps > 0:
                with torch.no_grad():
            ...
            # CuDNN LSTMs do not support double backward.
            # ----------------------------------------------------------
            opt_G.zero_grad()
            ...
            # Adversarial loss uses conditional discriminator
            ...
            # Feature matching (LSTM features only, replica-agnostic)
            ...
            # Variance regression: Huber loss pulling fake std -> real std per metric.
            # Replaces the adaptive cap system from S10.
            # - Pulls psi_cpu DOWN toward near-zero real std.
            # - Pulls throughput and gpu_util UP toward correct real std.
            # - No cap, no per-metric bookkeeping, single backward pass.
        ...
        if epoch % 10 == 0 or epoch == 1 or epoch == actual_warmup + 1:
            print(f"    ep {epoch:>4} [{phase_label}]  val_rec={val_rec:.5f}  "
                  ...
                  f"W={avg_wdist:.4f}  var_reg={h['var_reg']:.4f}  sm={h['smooth']:.4f}")
...
def main():
    parser = argparse.ArgumentParser(description="TimeGAN S11")
    parser.add_argument("--var-reg",     type=float, default=0.5,
                        help="Lambda for variance regression Huber loss (default 0.5)")
    ...
    run_tag    = f"s11_vr{int(args.var_reg * 10):02d}_ae{args.adv_epochs}{interp_tag}"

    out_dir   = Path(f"outputs/phase4/timegan_s11/{run_tag}")
    model_dir = Path(f"models/phase4/timegan_s11/{run_tag}")
    out_dir.mkdir(parents=True, exist_ok=True)
    model_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 70)
    print("TimeGAN S11 - Conditional Discriminator + Variance Regression")
    print("=" * 70)
    print(f"Device          : {device}")
    print(f"Var regression  : lambda={args.var_reg}  Huber delta=0.5  (no cap)")
    ...
    for workload in args.workloads:
        ...
        print(f"  Discriminator params: {n_params_D:,}  "
              f"(classifier input: {DISC_CFG['hidden_dim']*2+1} = hidden*2+1 for r_norm)")
        ...
        print(f"    var_ratio     = {vr_mean:.4f}  "
              f"(s10={ref_vr.get('s10','?'):.4f}  s9={ref_vr.get('s9','?'):.4f}  "
              f"lstm={ref_vr.get('lstm','?'):.4f})")
        print(f"    autocorr_diff = {ac_diff:.4f}")
        print(f"    jump_ratio    = {jump_ratio:.3f}x  "
              f"(s10={ref_jump.get('s10','?'):.3f}x  s9={ref_jump.get('s9','?'):.1f}x)")
