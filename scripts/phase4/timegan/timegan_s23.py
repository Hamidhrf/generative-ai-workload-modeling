"""
TimeGAN S21 - Segment-Based Generation with Precomputed FM Targets
========================================
...
S21: SEGMENT-BASED GENERATION + PRECOMPUTED FM TARGETS
================================
...
S21            THIS   segment-based generation; structural jump fix

USAGE
-----
    python timegan_s23.py
    python timegan_s23.py --workloads whisper yolo
    python timegan_s23.py --var-reg 0.3 --fm-stat 1.0 --adv-epochs 150

OUTPUT
------
    outputs/phase4/timegan_s23/{run_tag}/plots/
    models/phase4/timegan_s23/{run_tag}/{workload}/
    outputs/phase4/timegan_s23/{run_tag}/results.json

    run_tag = s23_seg_vr{var_reg*10}_fm{fm_stat*10}_ae{adv_epochs}
"""
...
WORKLOAD_OVERRIDES = {
    # S21 tuning: lambda_var_reg up for BERT/ResNet152 (within-segment variance recovery)
    # lambda_fm_stat up for GPT2 (scale-invariant loss now stable) and YOLO (reduce jump overshoot)
...
REFERENCE_VR = {
    "bert":      {"lstm": 0.594, "s9": 1.213, "s19": 1.226, "s21": 0.653},
    "gpt2":      {"lstm": 0.676, "s9": 0.664, "s19": 1.011, "s21": 1.015},
    "resnet152": {"lstm": 0.607, "s9": 0.324, "s19": 1.024, "s21": 0.990},
    "whisper":   {"lstm": 0.748, "s9": 0.743, "s19": 0.788, "s21": 1.722},
    "yolo":      {"lstm": 0.438, "s9": 1.446, "s19": 1.228, "s21": 0.630},
}

REFERENCE_JUMP = {
    "bert":      {"s8":  3.9, "s9":  8.7, "s19": 0.083, "s21": 1.540},
    "gpt2":      {"s8":  3.0, "s9":  7.2, "s19": 0.158, "s21": 3.199},
    "resnet152": {"s8":  6.5, "s9": 17.1, "s19": 0.095, "s21": 0.345},
    "whisper":   {"s8": 42.1, "s9": 26.2, "s19": 0.285, "s21": 2.793},
    "yolo":      {"s8": 10.6, "s9": 11.2, "s19": 0.122, "s21": 1.941},
}
...
class GeneratorSeg(nn.Module):
    """
    S21 generator: produces one phase segment of SEGMENT_LEN=120 steps.
    ...
    720-step trace; in S21, phase_idx is constant throughout a segment.
    """
...
# ------------------------------------------------------------------
# S21 fm_stat: precomputed population-level targets + scale-invariant loss
# ------------------------------------------------------------------

def precompute_fm_targets(segments, seg_rc, seg_phase_idx, device):
    """
    Compute per-(phase_idx, r_int) mean and std from ALL training segments.

    S20 computed statistics per-batch: with batch=32 and 42 groups, most
    batches had 0-1 samples per group triggering mask.sum()<2 skip -> sparse
    gradients -> VR degradation. S21 precomputes on full training set.

    Returns dict: (ph_int, r_int) -> {mean: Tensor(M), std: Tensor(M)+floor}
    """
    targets = {}
    for ph_val in set(seg_phase_idx.tolist()):
        for r_val in set(seg_rc.tolist()):
            ...
            if mask.sum() < 2:
                continue
            group = segments[mask]   # (n, seg_len, M)
            targets[(int(ph_val), int(r_val))] = {
                ...
                'count': int(mask.sum()),  # S23: group-size weighting
            }
    return targets
...
def compute_segment_fm_stat(fake, r_int, phase_idx_batch, fm_targets, device):
    """
    S21 fm_stat using precomputed population-level targets.

    Mean matching is scale-invariant: normalised by (|target_mean|+target_std)
    so that low-range metrics like GPT2 psi_cpu (0-0.002) contribute equally.
    Std matching uses log-ratio (already scale-invariant).

    fake            : (B, SEGMENT_LEN, M)
    r_int           : (B,) integer replica counts
    phase_idx_batch : (B,) integer phase indices 0..5
    fm_targets      : dict from precompute_fm_targets()
    """
    ...
    for r_val in unique_r:
        for ph_val in unique_ph:
            ...
            fake_seg  = fake[mask]                     # (n, seg_len, M)
            t_mean    = fm_targets[key]['mean']         # (M,) precomputed
            t_std     = fm_targets[key]['std']          # (M,) precomputed + floor
            grp_count = fm_targets[key].get('count', 4)  # S23: group sample count
            w = grp_count ** 0.5

            # Scale-invariant mean matching
            fake_mean  = fake_seg.mean(dim=(0, 1))
            scale      = (t_mean.abs() + t_std + 1e-6).detach()
            mean_loss  = ((fake_mean - t_mean.detach()) / scale).pow(2).mean()

            # Log-ratio std matching
            fake_std   = fake_seg.std(dim=(0, 1)) + 1e-8
            log_std    = torch.log(fake_std / t_std.detach())
            std_loss   = (log_std ** 2).mean()

            total_loss   = total_loss + w * (mean_loss + std_loss)
            total_weight += w
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for target, r_int, r_norm, phase_idx in train_dl:
            ...
            if lambda_var_reg > 0:
                ...
                if var_reg_metric_weights is not None and metric_names is not None:
                    w_m = torch.tensor(
                        [var_reg_metric_weights.get(m, 1.0) for m in metric_names],
                        dtype=torch.float32, device=device)
                    var_reg = (w_m * log_ratio ** 2).sum() / w_m.sum()
                else:
            ...
            # S21: uses precomputed fm targets (scale-invariant)
            if lambda_fm_stat > 0:
...
def evaluate(generator, raw_kept, replica_counts, phase_seq,
    ...
    """
    Evaluation for S21 segment-based model.
...
# ------------------------------------------------------------------
# Visualisation (adapted for S21 -- plot full 720-step traces)
# ------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(description="TimeGAN S21 segment-based")
    ...
    run_tag  = (f"s23_seg_vr{int(args.var_reg * 10):02d}"
                f"_fm{int(args.fm_stat * 10):02d}"
                f"_ae{args.adv_epochs}")
    out_dir  = Path(f"outputs/phase4/timegan_s23/{run_tag}")
    model_dir = Path(f"models/phase4/timegan_s23/{run_tag}")
    ...
    print("=" * 70)
    print("TimeGAN S21 - Segment-Based Generation with Precomputed FM Targets")
    print("=" * 70)
    ...
    for workload in args.workloads:
        ...
        # ------- S21 CORE: segment the full traces --------
        (all_segs, all_seg_rc,
        ...
        var_reg_weights_wl = wl_overrides.get('var_reg_weights', None)
        t0 = time.time()
        generator, discriminator, best_stat, n_epochs, history = train(
            ...
            var_reg_metric_weights=var_reg_weights_wl,
    ...
    # Summary table
    print(f"\n{'=' * 72}")
    print(f"S23 SUMMARY  ({run_tag})")
    print(f"{'=' * 72}")
    print(f"  {'Workload':<12} {'VR S23':>8} {'VR S21':>8} {'VR S19':>8} "
          f"{'LSTM':>8} {'jump S23':>10} {'jump S21':>10}")
    print(f"  {'-' * 72}")
    vr_means = []
