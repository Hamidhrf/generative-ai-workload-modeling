#!/usr/bin/env python3
"""
TimeGAN S9 - Per-Metric Variance Control + Fixed Augmentation
==============================================================
...
S8 RESULTS RECAP
================
Mean VR jumped from 0.415 (S7) to 1.678 (S8). Enc-dropout broke the
reconstruction lock. The adversarial game ran for 150 epochs and W-dist
rose steadily. This was the biggest improvement in the ablation (4x).

S8 REMAINING PROBLEMS
======================

1. OVER-GENERATION ON LOW-VARIANCE METRICS (VR capped at 5.0)
   BERT psi_cpu=5.0, latency=5.0; YOLO psi_cpu=5.0
   The generator produces too much variance on metrics where real
   variance is near-zero. The discriminator cannot distinguish
   "correct variance" from "too much variance" because both make the
   sequence look different from the near-flat real sequence. The
   adversarial signal pushes the generator to produce any deviation,
   not specifically the right amount.

   FIX: Per-metric variance penalty added to the generator loss.
   For each metric j, compute var_ratio_j = var(fake[:,j]) / var(real[:,j]).
   If var_ratio_j > variance_cap (default 2.0), add a penalty:
     penalty_j = (var_ratio_j - variance_cap)^2
   This is a differentiable term that penalises the generator when it
   over-generates on any single metric, without affecting metrics where
   VR is below the cap.
   lambda_var_pen controls the strength (default 0.5).

2. WHISPER JUMP RATIO 42x (temporal incoherence)
   Whisper has continuous dynamics without sharp phase transitions.
   Enc-dropout + adversarial pressure created random high-frequency
   spikes in the generator output.

   FIX A: Lower enc-dropout for Whisper specifically.
   The per-workload config allows enc_dropout=0.15 for Whisper vs
   0.3 for others. Less encoder corruption -> fewer random spikes.

   FIX B: Temporal smoothing penalty on the generator output.
   Add a loss term that penalises excessive timestep-to-timestep change
   in the synthetic traces: smooth_loss = mean(|fake[t] - fake[t-1]|^2).
   Weighted by lambda_smooth (default 0.1 for all, 0.3 for Whisper).
   This does not prevent the generator from producing variance -- it
   only discourages random uncorrelated noise between adjacent timesteps,
   which is what causes the high jump ratio.

3. REPLICA INTERPOLATION PRODUCED 0 NEW SAMPLES IN S8
   The function ran but produced 0 samples because replica_counts in
   the npz file has one value per EXPERIMENT (e.g. 5 values for 5
   r-values) rather than one value per POD (55 values). When train_idx
   (pod-level indices 0..48) indexes into a 5-element array, it causes
   wrong lookups that result in n=0 for every pair.

   FIX: Load replica_counts defensively. If len(replica_counts) != N_pods,
   reconstruct it from metadata. Also add a diagnostic print so the
   mismatch is visible at runtime.

   Additionally: for datasets with r=1,2,3,5,6,8,10 (consecutive pairs
   differ by 1), the old skip condition r_mid==r_lo or r_mid==r_hi fires
   for half the pairs. Replace with: skip if r_mid already exists in
   unique_r (meaning a real measurement exists at that r-value already
   and interpolation would just add noise rather than fill a gap).

ARCHITECTURE UNCHANGED FROM S8
================================
...
Lambda:        rec=0.1, adv=1.0 (same as S8)
New:           lambda_var_pen=0.5, lambda_smooth=0.1 (0.3 for Whisper)

PER-WORKLOAD DEFAULTS
======================
bert:      enc_dropout=0.3, lambda_smooth=0.1
gpt2:      enc_dropout=0.3, lambda_smooth=0.1
resnet152: enc_dropout=0.3, lambda_smooth=0.1
whisper:   enc_dropout=0.15, lambda_smooth=0.3
yolo:      enc_dropout=0.3, lambda_smooth=0.1

USAGE
-----
    python timegan_s9.py                         # all workloads, defaults
    python timegan_s9.py --workloads bert gpt2   # subset
    python timegan_s9.py --var-pen 0.0           # disable variance penalty
    python timegan_s9.py --smooth 0.0            # disable smoothing
    python timegan_s9.py --enc-dropout 0.3       # override all workloads

OUTPUT
------
    outputs/phase4/timegan_s9/{run_tag}/plots/
    models/phase4/timegan_s9/{run_tag}/{workload}/
    outputs/phase4/timegan_s9/{run_tag}/results.json

    run_tag = vp{var_pen}_sm{smooth}_ed{enc_dropout}
    e.g.      vp05_sm01_ed03
"""
...
# Per-workload enc_dropout and lambda_smooth overrides
# Whisper gets lower dropout and higher smoothing due to continuous dynamics
WORKLOAD_OVERRIDES = {
    "bert":      {"enc_dropout": 0.3,  "lambda_smooth": 0.1},
    "gpt2":      {"enc_dropout": 0.3,  "lambda_smooth": 0.1},
    "resnet152": {"enc_dropout": 0.3,  "lambda_smooth": 0.1},
...
REFERENCE_VR = {
    "bert":      {"lstm": 0.594, "s7": 0.240, "s8": 2.645},
    "gpt2":      {"lstm": 0.676, "s7": 0.573, "s8": 2.315},
    "resnet152": {"lstm": 0.607, "s7": 0.246, "s8": 1.055},
    "whisper":   {"lstm": 0.748, "s7": 0.745, "s8": 1.042},
    "yolo":      {"lstm": 0.438, "s7": 0.270, "s8": 1.334},
}

REFERENCE_JUMP = {
    "bert":      {"s7":  12.7, "s8":  3.9},
    "gpt2":      {"s7":  10.5, "s8":  3.0},
    "resnet152": {"s7":  12.6, "s8":  6.5},
    "whisper":   {"s7":  16.2, "s8": 42.1},
    "yolo":      {"s7":  10.2, "s8": 10.6},
}
...
def replica_interpolation(zm_traces, zm_with_phase, trace_means,
    ...
    """
    For consecutive replica count pairs in the training set where no real
    data exists at the midpoint, create interpolated training samples.

    Fixed vs S8:
    - Skip condition is now: r_mid already in unique_r (real data exists there)
      rather than r_mid == r_lo or r_mid == r_hi.
    - Added diagnostic prints to confirm samples are created.
    """
    train_r  = replica_counts[train_idx]
    unique_r = set(np.unique(train_r).tolist())
    ...
    new_r     = []
    new_eids  = []
    max_eid   = int(experiment_ids.max()) + 10000
    ...
    for i in range(len(unique_r_sorted) - 1):
        ...
        # Skip if a real measurement already exists at r_mid
        if r_mid in unique_r:
            continue
        # Skip if midpoint is not between endpoints (edge case with rounding)
    ...
    if not new_zm:
        print(f"    [interp] 0 samples created. "
              f"unique_r={sorted(unique_r)} -- all midpoints already covered.")
    ...
    ti_ext   = np.concatenate(
        [train_idx, np.arange(n_orig, n_orig + n_new, dtype=np.int64)])
...
# ------------------------------------------------------------------
# Dataset with phase boundary jitter (unchanged from S8)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Generator with encoder input dropout (same architecture as S8)
# ------------------------------------------------------------------
    ...
    def forward(self, x_with_phase, r_norm, exp_ids, phase_seq_batch):
        ...
        if self.enc_dropout_active and self.enc_dropout_p > 0 and self.training:
            mask   = torch.bernoulli(
                torch.full((B, enc_in.size(1), 1),
                           1.0 - self.enc_dropout_p, device=enc_in.device))
            enc_in = enc_in * mask
...
#
# New vs S8:
#   1. Per-metric variance penalty on generator loss (post-warmup).
#   2. Temporal smoothing penalty on generator output (post-warmup).
#   3. Both penalties use real batch statistics computed each step.
# ------------------------------------------------------------------

def train(generator, discriminator, train_dl, val_dl,
          base_cfg, disc_warmup, adv_epochs, gen_cfg,
          lambda_var_pen, variance_cap, lambda_smooth,
    ...
    print(f"    Adv:    {adv_epochs} fixed epochs  "
          f"var_pen={lambda_var_pen} smooth={lambda_smooth}")
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for inp, target, _, r_norm, exp_ids, ph_batch in train_dl:
            ...
            # --- NEW: Per-metric variance penalty ---
            # Applied only during adversarial phase (not warmup)
            # Penalises when synthetic variance exceeds real variance by > variance_cap
            if lambda_var_pen > 0 and not in_warmup:
                # Compute per-metric variance over batch x time
                var_real = target.var(dim=[0, 1]).detach()  # shape (M,)
                var_fake = fake.var(dim=[0, 1])              # shape (M,) differentiable
                # Ratio: clamp denominator to avoid division by near-zero
                var_ratio = var_fake / (var_real.clamp(min=1e-8))
                # Penalty when var_ratio > variance_cap
                excess    = F.relu(var_ratio - variance_cap)
                var_pen   = (excess ** 2).mean()
                loss_G    = loss_G + lambda_var_pen * var_pen
                ep_vpen  += var_pen.item()

            # --- NEW: Temporal smoothing penalty ---
            # Penalises large timestep-to-timestep changes in fake traces
            # Applied only during adversarial phase
            if lambda_smooth > 0 and not in_warmup:
                # diff shape: (B, T-1, M)
                diff       = fake[:, 1:, :] - fake[:, :-1, :]
                smooth_pen = (diff ** 2).mean()
                loss_G     = loss_G + lambda_smooth * smooth_pen
                ep_sm     += smooth_pen.item()
...
# ------------------------------------------------------------------
# Visualization (adds var_pen and smooth to loss plot)
# ------------------------------------------------------------------
...
def plot_comparison(workload, real_orig, syn_orig,
    ...
    boundaries  = [t for t in range(1, len(phase_seq)) if phase_seq[t] != phase_seq[t - 1]]
...
def main():
    parser = argparse.ArgumentParser(description="TimeGAN S9")
    parser.add_argument("--var-pen",     type=float, default=0.5,
                        help="Lambda for per-metric variance penalty (0=disabled)")
    parser.add_argument("--variance-cap",type=float, default=2.0,
                        help="VR threshold above which penalty fires (default 2.0)")
    parser.add_argument("--smooth",      type=float, default=None,
                        help="Lambda for temporal smoothing (None=use per-workload defaults)")
    parser.add_argument("--enc-dropout", type=float, default=None,
                        help="Override enc_dropout for all workloads (None=use per-workload defaults)")
    ...
    run_tag    = (f"vp{int(args.var_pen*10):02d}"
                  f"_vc{int(args.variance_cap*10):02d}"
                  f"_ae{args.adv_epochs}{interp_tag}")
    out_dir   = Path(f"outputs/phase4/timegan_s9/{run_tag}")
    model_dir = Path(f"models/phase4/timegan_s9/{run_tag}")
    out_dir.mkdir(parents=True, exist_ok=True)
    model_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 65)
    print("TimeGAN S9 - Variance Penalty + Smoothing + Fixed Augmentation")
    print("=" * 65)
    print(f"Device          : {device}")
    print(f"Var penalty     : lambda={args.var_pen}  cap={args.variance_cap}")
    ...
    for workload in args.workloads:
        ...
        enc_dropout_p   = args.enc_dropout if args.enc_dropout is not None else wl_overrides["enc_dropout"]
        lambda_smooth   = args.smooth      if args.smooth      is not None else wl_overrides["lambda_smooth"]
        ...
        # Validate replica_counts shape (fix S8 bug)
        replica_counts = validate_replica_counts(replica_counts, n_pods, metadata, workload)
        ...
        # Replica interpolation (fixed skip condition)
        if args.aug_interp:
        ...
        generator, discriminator, best_val_rec, n_epochs, history = train(
            ...
            args.var_pen, args.variance_cap, lambda_smooth,
        ...
        print(f"    var_ratio     = {vr_mean:.4f}  (s8={ref_vr.get('s8','?'):.4f}  lstm={ref_vr.get('lstm','?'):.4f})")
        print(f"    autocorr_diff = {ac_diff:.4f}")
        print(f"    jump_ratio    = {jump_ratio:.3f}x  (s8={ref_jump.get('s8','?'):.1f}x)")
        ...
        all_results.append({
            "workload": workload, "stage": "s9", "run_tag": run_tag,
            "enc_dropout": enc_dropout_p, "adv_epochs": args.adv_epochs,
            "disc_warmup": args.disc_warmup, "jitter": args.jitter,
            "aug_interp": args.aug_interp,
            "lambda_var_pen": args.var_pen, "variance_cap": args.variance_cap,
            "lambda_smooth": lambda_smooth,
    ...
    # Summary
    print(f"\n{'='*72}")
    print(f"S9 SUMMARY  ({run_tag})")
    print(f"{'='*72}")
    print(f"  {'Workload':<12}  {'VR S9':>7}  {'VR S8':>7}  {'LSTM':>7}  "
          f"{'jump S9':>9}  {'jump S8':>9}")
    print("  " + "-" * 62)
    vr_vals = []
    for r in all_results:
        ...
        print(f"  {r['workload']:<12}  {r['var_ratio_mean']:>7.4f}  "
              ...
              f"{r.get('ref_jump_s8', float('nan')):>8.1f}x")
    print("  " + "-" * 62)
    print(f"  {'MEAN':<12}  {float(np.mean(vr_vals)):>7.4f}")
    print(f"\nReference means:  LSTM=0.612  S7=0.415  S8=1.678")
    print(f"\nDiagnostic guide:")
    print(f"  VR > 3.0 on any metric: var_pen too low, try --var-pen 1.0")
    print(f"  VR < 0.5 on most metrics: var_pen too high, try --var-pen 0.2")
    print(f"  Whisper jump still > 20x: smooth too low, try --smooth 0.5")
    print(f"  Whisper VR collapsed: smooth too high, reduce to 0.1")
