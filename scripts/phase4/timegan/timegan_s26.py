"""
TimeGAN S26 - Segment-Based Generation + Correctly Normalised Autocorrelation Loss
========================================
Master Thesis - Generative AI Workload Modeling
Fachhochschule Dortmund

S26 vs S25: THREE CHANGES
===========================

Fix 1 - autocorr_loss normalisation bug (critical, S25 overcorrected):
  S25 normalised by fake_seg.std(), which is near-zero at epoch 1 because
  the untrained generator outputs values clustered near 0.5. This caused
  ac_loss values of 5,000,000+ at epoch 1, completely destroying the warmup
  phase (confirmed: BERT ac=5214047, Whisper ac=6865832 at ep 1). Whisper
  VR collapsed from 1.493 to 0.226 as a direct result.

  S26 fix: normalise BOTH real and fake by the REAL data's per-metric variance.
  This means:
    - ac_real is always in [-1, 1] (proper autocorrelation)
    - ac_fake is anchored to the same scale as ac_real
    - When the generator is untrained (output near real mean): fake_c ~= 0,
      ac_fake ~= 0, loss per lag ~= ac_real^2 ~= 0.25, total ~= 0.025 at
      lambda_ac=0.1. Fully compatible with fm_stat (~0.5-2.0).
    - When the generator is trained well: ac_fake ~= ac_real, loss -> 0.

Fix 2 - Whisper lambda_fm_stat reverted 0.8 -> 0.5:
  S25 tightened this to counter CPU level drift, but the dominant failure was
  the ac explosion. With the ac loss fixed, reverting to the S21/S24 value
  that produced VR=1.284/1.493. Will re-tighten if drift reappears in S27.

Fix 3 - YOLO n_disc_steps=1 and ResNet152 lambda_smooth=0.2 KEPT from S25:
  Both changes were independently confirmed effective:
  - YOLO: VR 0.606 -> 0.788, adv_gen stayed bounded
  - ResNet152: autocorr_diff improved 0.241 -> 0.096 (best across all stages)

S25 RESULTS (starting point for S26)
======================================
  bert      VR=0.485  (ac exploded during warmup, underfitting)
  gpt2      VR=0.873  (ac exploded during warmup, underfitting)
  resnet152 VR=0.710  autocorr_diff=0.096 (good, but VR regressed)
  whisper   VR=0.226  (ac explosion destroyed training, VR collapsed)
  yolo      VR=0.788  (n_disc=1 fix worked, jump_ratio=5.95 borderline)
  MEAN      0.616

S26 TARGETS
============
1. ac_loss should show ~0.01-0.1 range throughout (not millions, not 0.0001)
2. Whisper VR should recover toward S21/S24 level (>1.0)
3. BERT/GPT2 VR should recover toward S21 level
4. ResNet152 autocorr_diff should remain low (<0.15)
5. YOLO adv_gen should remain bounded
6. Mean VR >= S21 (0.9146)

USAGE
-----
    python timegan_s26.py
    python timegan_s26.py --workloads bert yolo
    python timegan_s26.py --lambda-ac 0.1
    python timegan_s26.py --var-reg 0.3 --fm-stat 1.0 --adv-epochs 150

OUTPUT
------
    outputs/phase4/timegan_s26/{run_tag}/plots/
    models/phase4/timegan_s26/{run_tag}/{workload}/
    outputs/phase4/timegan_s26/{run_tag}/results.json

    run_tag = s26_seg_vr{var_reg*10}_fm{fm_stat*10}_ae{adv_epochs}_ac{ac*10}
"""
...
# S26: whisper lambda_fm_stat reverted 0.8 -> 0.5 (S21 value)
#      resnet152 lambda_smooth=0.2 kept from S25 (confirmed effective)
#      yolo n_disc_steps=1 kept from S25 (confirmed effective)
WORKLOAD_OVERRIDES = {
...
REFERENCE_VR = {
    "bert":      {"lstm": 0.594, "s19": 1.226, "s21": 0.672, "s24": 0.733, "s25": 0.485},
    "gpt2":      {"lstm": 0.676, "s19": 1.011, "s21": 0.996, "s24": 1.082, "s25": 0.873},
    "resnet152": {"lstm": 0.607, "s19": 1.024, "s21": 0.960, "s24": 0.747, "s25": 0.710},
    "whisper":   {"lstm": 0.748, "s19": 0.788, "s21": 1.284, "s24": 1.493, "s25": 0.226},
    "yolo":      {"lstm": 0.438, "s19": 1.228, "s21": 0.661, "s24": 0.606, "s25": 0.788},
}
...
def autocorr_loss(real_seg, fake_seg, max_lag=20):
    """
    Autocorrelation matching loss (S26 corrected version).

    Both inputs: (B, T, M) -- 120-step segments.
    real_seg is detached so gradients only flow through fake_seg.

    S25 BUG: normalised by fake_seg.std(). At epoch 1 the untrained
    generator outputs values near 0.5 with near-zero variance, so
    fake_std ~= 0.003 and fake_n values ~= 167. This caused ac_loss
    values of 5,000,000+ at epoch 1, destroying the warmup phase
    (confirmed: BERT ac=5214047, Whisper ac=6865832 at ep 1).

    S26 FIX: normalise BOTH real and fake by the REAL data's per-metric
    variance. This means:
      - ac_real is always in [-1, 1]  (proper normalised autocorrelation)
      - ac_fake is anchored to the same scale as ac_real
      - At epoch 1 (untrained generator, output near real mean):
          fake_c ~= 0  =>  ac_fake ~= 0
          loss per lag ~= ac_real^2 ~= 0.25
          total ~= 0.025 at lambda_ac=0.1  (compatible with fm_stat)
      - When well-trained: ac_fake ~= ac_real  =>  loss -> 0

    Loss is averaged over lags so magnitude is independent of max_lag.
    """
    real_d    = real_seg.detach()
    real_mean = real_d.mean(dim=[0, 1], keepdim=True)                # (1, 1, M)
    real_var  = ((real_d - real_mean) ** 2).mean(dim=[0, 1]) + 1e-8  # (M,)
    real_c    = real_d   - real_mean
    fake_c    = fake_seg - real_mean           # centre fake by real mean
    loss = torch.tensor(0.0, device=fake_seg.device)
    for lag in range(1, max_lag + 1):
        cov_real = (real_c[:, :-lag, :] * real_c[:, lag:, :]).mean(dim=[0, 1])
        cov_fake = (fake_c[:, :-lag, :] * fake_c[:, lag:, :]).mean(dim=[0, 1])
        ac_real  = cov_real / real_var    # in [-1, 1]
        ac_fake  = cov_fake / real_var    # same scale as ac_real
        loss     = loss + ((ac_real - ac_fake) ** 2).mean()
    return loss / max_lag
...
def train(generator, discriminator, train_dl, val_dl,
    ...
    Identical to S24 except:
      - autocorr_loss uses real-variance-normalized signals (S26 fix)
      - per-workload overrides: resnet152 smooth=0.2, yolo n_disc=1, whisper fm=0.5
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for target, r_int, r_norm, phase_idx in train_dl:
            ...
            # S26: Autocorrelation loss with real-variance-normalised signals
            if lambda_ac > 0:
...
def main():
    parser = argparse.ArgumentParser(description="TimeGAN S26 segment-based + corrected autocorr loss")
    ...
    # S24/S25/S26: autocorrelation loss arguments
    ...
    run_tag   = (f"s26_seg_vr{int(args.var_reg * 10):02d}"
                 f"_fm{int(args.fm_stat * 10):02d}"
                 f"_ae{args.adv_epochs}"
                 f"_ac{int(args.lambda_ac * 10):02d}")
    out_dir   = Path(f"outputs/phase4/timegan_s26/{run_tag}")
    model_dir = Path(f"models/phase4/timegan_s26/{run_tag}")
    out_dir.mkdir(parents=True, exist_ok=True)

    boundaries = DEFAULT_PHASE_BOUNDARIES

    print("=" * 70)
    print("TimeGAN S26 - Segment-Based Generation + Corrected Autocorrelation Loss")
    print("=" * 70)
    ...
    for workload in args.workloads:
        ...
        print(f"    var_ratio     = {vr_mean:.4f}  "
              ...
              f"s25={ref_vr.get('s25', '?')}  lstm={ref_vr.get('lstm', '?')})")
        ...
        result = {
            ...
            "ref_vr_s24":        ref_vr.get("s24"),
            "ref_vr_s25":        ref_vr.get("s25"),
    ...
    print(f"\n{'=' * 72}")
    print(f"S26 SUMMARY  ({run_tag})")
    print(f"{'=' * 72}")
    print(f"  {'Workload':<12} {'VR S26':>8} {'VR S21':>8} {'VR S24':>8} "
          f"{'LSTM':>8} {'jump S26':>10}")
    print(f"  {'-' * 72}")
    vr_means = []
