"""
E6 fm-weight search on the low-data-budget failures (NOT committed).

Generic single-(tier, workload, fm) trainer, adapted from
timegan_s36_tier2_sweep.py / timegan_s36_tier3_sweep.py's --workload/--vr/--fm
CLI-override pattern. Only lambda_fm_stat is overridden per run; every other
S27_HYPERPARAMS entry for the workload (lambda_var_reg, lambda_smooth,
n_disc_steps) plus architecture, GEN_CFG/DISC_CFG (including the unused-but-
frozen GEN_CFG["seed"]=42 marker), epoch schedule (warmup=20, adv=150), and
loss terms are byte-identical to timegan_s36_tier3.py / timegan_s36_tier2.py
/ timegan_s36_tier3_matched.py -- this is the frozen recipe with one
hyperparameter re-searched, not a new architecture or training loop.

--tier selects the data source:
  tier2         -> data/processed/tier2/unified          (MIG, n=28/workload)
  tier3_matched -> data/processed/tier3_matched/unified   (time-slicing, n=28/workload, E1 dataset)
Neither is written to; both are read-only inputs already produced by prior
steps (committed Tier 2 preprocessing; E1's tier3_matched preprocessing).

Output: models/phase4/timegan_s36_<tier>_e6_fm<fmtag>/<workload>/generator.pt
where fmtag is the fm value with '.' -> 'p' (e.g. fm=0.125 -> fm0p125).
This never collides with frozen S36, committed Tier 2/3, or the E1
tier3_matched checkpoint dirs (all different path prefixes).

Usage:
    python timegan_s36_e6_fmsearch.py --tier tier2 --workload gpt2 --fm 0.25
"""
...
# S27 hyperparameters (frozen). Only lambda_fm_stat is overridden per E6 run.
S27_HYPERPARAMS = {
    "bert": {
        "lambda_var_reg": 0.5,
        "lambda_fm_stat": 1.5,
...
def load_workload_data(workload_name, data_combined, norm_combined):
...
class GeneratorSeg(nn.Module):
    """S27 generator architecture - IDENTICAL TO S34/S36."""
...
class DiscriminatorSpectral(nn.Module):
    """S36 discriminator with spectral normalization."""
...
def train_workload(workload_name, hyperparams, device, data_combined, norm_combined):
    print(f"\n{'='*80}")
    print(f"E6 TRAINING: {workload_name.upper()}  (data={data_combined})")
    print(f"{'='*80}")

    traces, rc, train_idx, val_idx, norm, kept_names = load_workload_data(
        workload_name, data_combined, norm_combined)
...
def main():
    parser = argparse.ArgumentParser(description="E6 - fm-weight search on low-data-budget failures")
    parser.add_argument("--tier", type=str, required=True, choices=list(TIER_DATA_PATHS.keys()))
    parser.add_argument("--workload", type=str, required=True, choices=WORKLOADS_ALL)
    parser.add_argument("--fm", type=float, required=True, help="Override lambda_fm_stat")
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    paths = TIER_DATA_PATHS[args.tier]
    hyperparams = dict(S27_HYPERPARAMS[args.workload])
    hyperparams["lambda_fm_stat"] = args.fm

    print("=" * 80)
    print("E6 FM-WEIGHT SEARCH (NOT COMMITTED)")
    print("=" * 80)
    print(f"Device: {device}")
    print(f"Tier: {args.tier}  Workload: {args.workload}  fm override: {args.fm}")
    print(f"Full hyperparams (only lambda_fm_stat overridden): {hyperparams}")
    print("=" * 80)

    t0 = time.time()
    generator, best_val = train_workload(
        args.workload, hyperparams, device, paths["data_combined"], paths["norm_combined"])
    elapsed = time.time() - t0

    tag = fmtag(args.fm)
    model_dir = Path(f"models/phase4/timegan_s36_{args.tier}_e6_{tag}/{args.workload}")
    model_dir.mkdir(parents=True, exist_ok=True)
    torch.save(generator.state_dict(), model_dir / "generator.pt")

    out_dir = Path(f"outputs/phase4/timegan_s36_{args.tier}_e6_{tag}")
    out_dir.mkdir(parents=True, exist_ok=True)
    result = {
        "tier": args.tier, "workload": args.workload, "fm_override": args.fm,
        "hyperparams": hyperparams, "best_val": float(best_val), "elapsed_s": elapsed,
        "model_path": str(model_dir),
    }
    with open(out_dir / f"{args.workload}_summary.json", "w") as f:
        json.dump(result, f, indent=2)

    print(f"\n{args.workload.upper()} @ {args.tier} fm={args.fm} complete: {elapsed:.1f}s, val={best_val:.5f}")
