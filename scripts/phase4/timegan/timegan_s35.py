"""
TimeGAN S35 - S34 + Data Augmentation (4x Dataset)
===================================================
...
ABLATION STUDY:
- Architecture: IDENTICAL to S34
- Hyperparameters: IDENTICAL to S34
- Training: IDENTICAL to S34
- ONLY CHANGE: Dataset size (176 -> 704 traces)

AUGMENTATION:
- Sliding window (3 segments): +352 traces
- Noise injection (sigma=0.02): +176 traces
- Total: 704 traces (4x original)

RATIONALE:
- Test if more training data improves VR
- Single-variable ablation (only data changed)
- If successful, proceed to S36 (spectral norm)

USAGE:
------
    python timegan_s35.py --workloads bert gpt2 resnet152 whisper yolo
    python timegan_s35.py --workloads whisper  # Train only Whisper

OUTPUT:
-------
    models/phase4/timegan_s35/s35_{workload}_vr{X}_fm{Y}/
    outputs/phase4/timegan_s35/s35_summary.json
"""
...
# ONLY CHANGE FROM S34: Use augmented dataset
...
class Discriminator(nn.Module):
    """S27 discriminator - IDENTICAL TO S34."""
...
def train_workload(workload_name, hyperparams, device):
    """Train S35 model for single workload - IDENTICAL TO S34."""
    print(f"\n{'='*80}")
    print(f"S35 TRAINING: {workload_name.upper()}")
    print(f"{'='*80}")
...
def main():
    parser = argparse.ArgumentParser(description="S35 - S34 + Data Augmentation (4x)")
    ...
    print("="*80)
    print("S35: S34 + DATA AUGMENTATION (4x DATASET)")
    print("="*80)
    print(f"Device: {device}")
    print(f"Workloads: {args.workloads}")
    print(f"Architecture: S34 (IDENTICAL)")
    ...
    for wl in args.workloads:
        ...
        # Save model
        tag = f"s35_{wl}_vr{int(hyperparams['lambda_var_reg']*10):02d}_fm{int(hyperparams['lambda_fm_stat']*10):02d}"
        model_dir = Path(f"models/phase4/timegan_s35/{tag}")
    ...
    with open(out_dir / "s35_summary.json", "w") as f:
        json.dump({
            "stage": "s35",
            "description": "S34 architecture, 4x augmented dataset (704 traces)",
            "workloads": args.workloads,
            "results": results,
            "augmentation": {
                "original_traces": 176,
                "augmented_traces": 704,
                "multiplier": 4,
                "methods": ["sliding_window", "noise_injection"]
            }
        }, f, indent=2)
    
    print("\n"+"="*80)
    print("S35 TRAINING COMPLETE")
    print("="*80)
    print(f"Summary: {out_dir / 's35_summary.json'}")
    print("\nNext: Evaluate S35 vs S34 with eval_s35.py")
