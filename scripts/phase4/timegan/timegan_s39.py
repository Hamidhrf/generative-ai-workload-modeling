"""
TimeGAN S39 - S36 + Targeted Variance Tuning
=============================================
Master Thesis - Generative AI Workload Modeling
Fachhochschule Dortmund

S39 = S36 Architecture + Tuned Variance Regularization
=======================================================

ABLATION STUDY: Does targeted hyperparameter tuning improve VR?
- S36 baseline: Mean VR = 1.116, but GPT2=1.137, Whisper=1.397 (too high)
- S39: Increase lambda_var_reg ONLY for problem workloads

TARGETED TUNING STRATEGY:
- GPT2: lambda_var_reg 0.3 → 0.6 (2x stronger variance penalty)
- Whisper: lambda_var_reg 0.5 → 0.8 (1.6x stronger variance penalty)
- BERT, ResNet152, YOLO: UNCHANGED (already working well)

RATIONALE:
- S36 has 2 workloads with excessive variance (GPT2, Whisper)
- Variance regularization directly controls VR
- Surgical fix: only change what's broken
- High win rate: 75-85% probability of improvement

EXPECTED OUTCOME:
- GPT2: 1.137 → 1.05-1.08
- Whisper: 1.397 → 1.20-1.25
- Mean VR: 1.116 → 1.06-1.09 (target: below 1.10!)

USAGE:
------
    python timegan_s39.py --workloads gpt2 whisper  # Only retrain problem workloads
    python timegan_s39.py --workloads gpt2          # Or one at a time
...
S27_HYPERPARAMS = {
    "bert": {
        "lambda_var_reg": 0.5,  # UNCHANGED (S36 VR=1.000 - perfect!)
        "lambda_fm_stat": 1.5,
        "lambda_smooth": 0.05,
        "n_disc_steps": 2,
    },
    "gpt2": {
        "lambda_var_reg": 0.6,  # CHANGED: 0.3->0.6 (S36 VR=1.137, target: 1.05-1.08)
        "lambda_fm_stat": 2.0,
        "lambda_smooth": 0.1,
        "n_disc_steps": 2,
    },
    "resnet152": {
        "lambda_var_reg": 0.4,  # UNCHANGED (S36 VR=0.985 - perfect!)
        "lambda_fm_stat": 1.0,
        "lambda_smooth": 0.1,
        "n_disc_steps": 2,
    },
    "whisper": {
        "lambda_var_reg": 0.8,  # CHANGED: 0.5->0.8 (S36 VR=1.397, target: 1.20-1.25)
        "lambda_fm_stat": 0.5,
        "lambda_smooth": 0.05,
        "n_disc_steps": 1,
    },
    "yolo": {
        "lambda_var_reg": 0.3,  # UNCHANGED (S36 VR=1.039 - good!)
        "lambda_fm_stat": 1.2,
        "lambda_smooth": 0.1,
        "n_disc_steps": 2,
    },
...
def main():
    ...
    for wl in args.workloads:
        ...
        # Save model
        tag = f"s39_{wl}_vr{int(hyperparams['lambda_var_reg']*10):02d}_fm{int(hyperparams['lambda_fm_stat']*10):02d}"
        model_dir = Path(f"models/phase4/timegan_s39/{tag}")
