#!/usr/bin/env python3
"""
TimeGAN S17 - Discriminator Gradient Clipping + Stronger Phase FM
===================================================================
...
S16 DIAGNOSIS
==============
S16 (mean VR=1.009) introduced per-phase fm_stat which fixed BERT throughput
(0.596->1.135) and improved GPT2 overall (0.775->0.943). However three new
problems appeared.

PROBLEM 1: YOLO DISCRIMINATOR EXPLOSION (disc=1600 at ep160)
  S16 applies clip_grad_norm_ to the generator (max_norm=1.0) but NOT to the
  discriminator. The discriminator gradient exploded to 1600 at epoch 160.
  YOLO throughput collapsed from VR=0.401 (S15) to VR=0.246 (S16).
  The best W-dist checkpoint (W=12.3) was taken near the explosion.

PROBLEM 2: WHISPER SEVERE REGRESSION (VR 0.838->0.543)
  n_disc_steps=2 still too strong for Whisper. W-dist stayed near 0 (0.28)
  throughout all 170 adversarial epochs. With W~0 the adversarial gradient is
  near-zero, leaving fm_stat + smooth as the only training signals. But
  lambda_smooth=0.3 over-regularises toward flat outputs. Result:
    cpu_usage VR=0.346, latency VR=0.290 (severely under-generated)

PROBLEM 3: LEVEL ERRORS PERSIST (ResNet152 latency 0.569, BERT cpu 0.727)
  Per-phase fm_stat at lambda=0.5 is not strong enough to override adversarial
  drift. Generator level errors remain for these metrics.

S17 FIX 1: DISCRIMINATOR GRADIENT CLIPPING (max_norm=5.0)
============================================================
Add nn.utils.clip_grad_norm_(discriminator.parameters(), 5.0) after each
discriminator backward pass. The generator already has clipping (max_norm=1.0);
the discriminator has none. This prevents YOLO disc explosion and any future
gradient explosions across all workloads.

S17 FIX 2: STRONGER PER-PHASE FM_STAT (lambda 0.5->1.0)
==========================================================
Double the per-phase statistical feature matching weight. This anchors
per-phase level targets more firmly against adversarial drift:
  - YOLO throughput: forces generator to match per-phase throughput levels
  - ResNet152 latency: prevents level under-generation
  - BERT cpu: anchors cpu level per phase
The per-phase constraint worked at 0.5 (BERT throughput fixed). At 1.0 it
should handle the remaining level errors without losing variance.

S17 FIX 3: WHISPER n_disc=1, lambda_smooth=0.05
==================================================
With W~0 and n_disc=2, Whisper's discriminator still dominates. Reducing to
n_disc=1 gives the generator one discriminator update per generator step.
Reducing lambda_smooth from 0.3 to 0.05 removes the over-regularisation that
suppresses cpu and latency variance.

CHANGES FROM S16
==================
Discriminator training:  add clip_grad_norm_(disc, 5.0) after backward
lambda_fm_stat default:  0.5 -> 1.0
Whisper n_disc_steps:    2 -> 1
Whisper lambda_smooth:   0.3 -> 0.05
run_tag:                 s16 -> s17
All other code:          identical to S16
...
S17            THIS   disc gradient clipping; fm_stat 0.5->1.0; Whisper n_disc=1

USAGE
-----
    python timegan_s17.py
    python timegan_s17.py --workloads bert yolo
    python timegan_s17.py --var-reg 0.3 --fm-stat 1.0 --adv-epochs 150

OUTPUT
------
    outputs/phase4/timegan_s17/{run_tag}/plots/
    models/phase4/timegan_s17/{run_tag}/{workload}/
    outputs/phase4/timegan_s17/{run_tag}/results.json

    run_tag = s17_vr{var_reg*10}_fm{fm_stat*10}_ae{adv_epochs}
"""
...
# S17: Whisper n_disc_steps 2->1, lambda_smooth 0.3->0.05
WORKLOAD_OVERRIDES = {
...
REFERENCE_VR = {
    "bert":      {"lstm": 0.594, "s9": 1.213, "s16": 1.485},
    "gpt2":      {"lstm": 0.676, "s9": 0.664, "s16": 0.943},
    "resnet152": {"lstm": 0.607, "s9": 0.324, "s16": 0.953},
    "whisper":   {"lstm": 0.748, "s9": 0.743, "s16": 0.543},
    "yolo":      {"lstm": 0.438, "s9": 1.446, "s16": 1.121},
}

REFERENCE_JUMP = {
    "bert":      {"s8":  3.9, "s9":  8.7, "s16": 0.075},
    "gpt2":      {"s8":  3.0, "s9":  7.2, "s16": 0.108},
    "resnet152": {"s8":  6.5, "s9": 17.1, "s16": 0.122},
    "whisper":   {"s8": 42.1, "s9": 26.2, "s16": 0.359},
    "yolo":      {"s8": 10.6, "s9": 11.2, "s16": 0.117},
}
...
def train(generator, discriminator, train_dl, val_dl,
    ...
    """
    S16 training - per-phase fm_stat replaces global fm_stat.
...
def main():
    ...
    out_dir   = Path(f"outputs/phase4/timegan_s17/{run_tag}")
    model_dir = Path(f"models/phase4/timegan_s17/{run_tag}")
    ...
    print(f"Per-phase FM    : lambda_fm_stat={args.fm_stat}  (S17: doubled from S16=0.5)")
    ...
    for workload in args.workloads:
        ...
        print(f"    var_ratio     = {vr_mean:.4f}  "
              f"(s16={ref_vr.get('s16','?'):.4f}  s9={ref_vr.get('s9','?'):.4f}  "
              f"lstm={ref_vr.get('lstm','?'):.4f})")
        print(f"    autocorr_diff = {ac_diff:.4f}")
        print(f"    jump_ratio    = {jump_ratio:.3f}x  "
              f"(s16={ref_jump.get('s16','?'):.3f}x  s9={ref_jump.get('s9','?'):.1f}x)")
