"""
TimeGAN S38 - S36 + Multi-Layer Feature Matching
=================================================
Master Thesis - Generative AI Workload Modeling
Fachhochschule Dortmund

S38 = S36 Architecture + Multi-Layer Feature Matching
======================================================

ABLATION STUDY: Does multi-layer feature matching improve VR?
- S36 baseline: Mean VR = 1.116 (spectral normalization)
- S38: S36 + Feature matching at multiple discriminator layers

MULTI-LAYER FEATURE MATCHING:
- Extract features at 3 levels: LSTM output, FC1 output, FC2 input
- Match statistics (mean, std) at each layer
- Captures hierarchical patterns:
  * LSTM features: Low-level temporal patterns
  * FC1 features: Mid-level representations
  * FC2 features: High-level decision features
- Proven in BigGAN, StyleGAN for improved generation quality

RATIONALE:
- S36 only matches segment-level statistics (high-level)
- Multi-layer FM adds discriminator feature matching (learned representations)
- Captures both explicit (segment stats) and implicit (learned) patterns
- Expected: Better temporal consistency, VR closer to 1.0

USAGE:
------
    python timegan_s38.py --workloads bert gpt2 resnet152 whisper yolo
    python timegan_s38.py --workloads bert  # Train only BERT

OUTPUT:
-------
    models/phase4/timegan_s38/s38_{workload}_vr{X}_fm{Y}/
    outputs/phase4/timegan_s38/s38_summary.json
"""
...
class DiscriminatorMultiLayer(nn.Module):
    """S38 discriminator: S36 (spectral norm) + multi-layer feature extraction"""
    def __init__(self, n_metrics, cfg):
        ...
        # S38: Break down classifier to expose intermediate features
        self.fc1 = nn.utils.spectral_norm(nn.Linear(hidden * 2 + 1, max(hidden, 16)))
        self.relu = nn.LeakyReLU(0.2)
        self.fc2 = nn.utils.spectral_norm(nn.Linear(max(hidden, 16), 1))
    
    def forward(self, x, r_norm=None, return_features=False):
        # LSTM layer - extracts temporal features
        _, (h_n, _) = self.lstm(x)
        h_lstm = torch.cat([h_n[-2], h_n[-1]], dim=1)
        
        # Concatenate replica count
        if r_norm is not None:
            h_cat = torch.cat([h_lstm, r_norm.unsqueeze(1)], dim=1)
        else:
            h_cat = torch.cat([h_lstm, torch.zeros(h_lstm.shape[0], 1, device=h_lstm.device)], dim=1)
        
        # FC layer 1 - mid-level features
        h1 = self.relu(self.fc1(h_cat))
        
        # FC layer 2 - final output
        out = self.fc2(h1)
        
        if return_features:
            # Return output + intermediate features at 3 levels
            return out, [h_lstm, h_cat, h1]
        return out
...
def train_workload(workload_name, hyperparams, device):
    """Train S38 model for single workload."""
    print(f"\n{'='*80}")
    print(f"S38 TRAINING: {workload_name.upper()}")
    print(f"{'='*80}")
    ...
    # Models - S38 uses multi-layer feature matching discriminator
    generator = GeneratorSeg(SEGMENT_LEN, n_metrics, GEN_CFG).to(device)
    discriminator = DiscriminatorMultiLayer(n_metrics, DISC_CFG).to(device)
    
    print(f"Generator: {sum(p.numel() for p in generator.parameters()):,} params")
    print(f"Discriminator: Spectral Norm + Multi-Layer Feature Matching")
    ...
    for epoch in range(1, total + 1):
        ...
        for target, r_int, r_norm, ph_idx in dl_train:
            ...
            if not in_warmup:
                # S38: Get discriminator features for multi-layer FM
                sf, fake_feats = discriminator(fake, r_norm, return_features=True)
                sr, real_feats = discriminator(target, r_norm, return_features=True)
                
                # Adversarial loss
                loss_G += -sf.mean()
                
                # S38: Multi-layer feature matching - NEW!
                lambda_mlayer_fm = 0.5  # Weight for multi-layer FM
                for f_fake, f_real in zip(fake_feats, real_feats):
                    # Match mean and std at each layer
                    loss_G += lambda_mlayer_fm * F.l1_loss(f_fake.mean(0), f_real.mean(0).detach())
                    loss_G += lambda_mlayer_fm * F.l1_loss(f_fake.std(0), f_real.std(0).detach())
...
def main():
    parser = argparse.ArgumentParser(description="S38 - S36 + Multi-Layer Feature Matching")
    ...
    print("="*80)
    print("S38: S36 + MULTI-LAYER FEATURE MATCHING")
    print("="*80)
    ...
    print(f"Improvement: Hierarchical feature matching")
    print("="*80)
    ...
    for wl in args.workloads:
        ...
        # Save model
        tag = f"s38_{wl}_vr{int(hyperparams['lambda_var_reg']*10):02d}_fm{int(hyperparams['lambda_fm_stat']*10):02d}"
        model_dir = Path(f"models/phase4/timegan_s38/{tag}")
    ...
    # Save summary
    out_dir = Path("outputs/phase4/timegan_s38")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    with open(out_dir / "s38_summary.json", "w") as f:
        json.dump({
            "stage": "s38",
            "description": "S36 architecture + Multi-Layer Feature Matching",
            "workloads": args.workloads,
            "improvement": "multi_layer_feature_matching",
            "baseline": "s36",
            "results": results,
        }, f, indent=2)
    
    print("\n"+"="*80)
    print("S38 TRAINING COMPLETE")
    print("="*80)
    print(f"Summary: {out_dir / 's38_summary.json'}")
    print("\nNext: Evaluate with eval_s38.py to compare vs S36")
