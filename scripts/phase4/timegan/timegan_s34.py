"""
TimeGAN S34 - S33 + Whisper (All 5 Workloads)
===============================================
Master Thesis - Generative AI Workload Modeling
Fachhochschule Dortmund

S34 = S33 Architecture + Whisper Added Back
============================================

RATIONALE:
- S33 showed excellent results for GPU-bound workloads
- Adding Whisper for completeness (all 5 workloads)
- Uses S27's proven hyperparameters for each workload

ARCHITECTURE:
- Per-workload models (5 separate TimeGANs)
- BERT, GPT2, ResNet152, Whisper, YOLO
- S27's proven hyperparameters per workload
- ALL 5 workloads have SAME 7 metrics (maximum consistency)

USAGE:
------
    python timegan_s34.py --workloads bert gpt2 resnet152 whisper yolo
    python timegan_s34.py --workloads whisper  # Train only Whisper

OUTPUT:
-------
    models/phase4/timegan_s34/s34_{workload}_vr{X}_fm{Y}/
    outputs/phase4/timegan_s34/s34_summary.json
"""
...
DROP_PER_WORKLOAD = {
    ...
    "whisper":   {"gpu_memory_total", "gpu_memory_used", "gpu_temperature"},  # Now same 7 metrics!
...
def train_workload(workload_name, hyperparams, device):
    """Train S34 model for single workload."""
    print(f"\n{'='*80}")
    print(f"S34 TRAINING: {workload_name.upper()}")
    print(f"{'='*80}")
...
def main():
    parser = argparse.ArgumentParser(description="S34 - S33 + Whisper (All 5 Workloads)")
    ...
    print("="*80)
    print("S34: S33 + WHISPER (ALL 5 WORKLOADS)")
    print("="*80)
    ...
    for wl in args.workloads:
        ...
        # Save model
        tag = f"s34_{wl}_vr{int(hyperparams['lambda_var_reg']*10):02d}_fm{int(hyperparams['lambda_fm_stat']*10):02d}"
        model_dir = Path(f"models/phase4/timegan_s34/{tag}")
    ...
    with open(out_dir / "s34_summary.json", "w") as f:
        json.dump({
            "stage": "s34",
            "description": "S27 architecture, all 5 workloads (S33 + Whisper)",
            "workloads": args.workloads,
            "results": results,
        }, f, indent=2)
    
    print("\n"+"="*80)
    print("S34 TRAINING COMPLETE")
    print("="*80)
    print(f"Summary: {out_dir / 's34_summary.json'}")
    print("\nNext: Evaluate with eval_s34.py")
