#!/usr/bin/env python3
"""
TimeGAN Stage 3 - Richer Generator Loss (Feature Matching + Moment Matching)
Master Thesis - Generative AI Workload Modeling
Fachhochschule Dortmund

WHAT CHANGED FROM STAGE 2
==========================
Stage 2 result: mean VR = 0.511 vs LSTM 0.612.
Stabilization confirmed working (S1=0.371 -> S2=0.511).

Remaining failure mode: synthetic traces are smoother/flatter than real.
Visible in plots as dashed lines (synthetic) running below solid lines (real)
in variance. The generator is still rewarded for predicting conditional means
because MSE loss is dominated by the mean.

This is NOT a stability problem anymore (S2 solved that for most workloads).
It is a LOSS SIGNAL problem: the generator is not told to match variance.

Two additions that directly target variance undershoot:

1. FEATURE MATCHING (Salimans et al. 2016)
   L_fm = MSE( mean(D_feat(fake)), mean(D_feat(real)).detach() )
   Discriminator hidden features encode temporal structure. Matching their
   mean forces the generator to produce traces whose internal statistics
   resemble real traces, not just traces that fool the scalar output.
   Requires D.get_features(x) to expose the pre-classifier representation.

2. MOMENT MATCHING on raw traces
   L_var  = MSE( var(fake, dim=0), var(real, dim=0).detach() )
   L_auto = MSE( autocorr(fake, lag=1), autocorr(real, lag=1).detach() )
   var(dim=0): per-timestep variance across the batch. Non-zero if generator
   collapses all outputs to the mean.
   autocorr lag=1: penalizes traces that are too smooth (over-correlated)
   or too noisy (under-correlated) relative to real.
...
Discriminator loss unchanged from Stage 2.
Architecture unchanged from Stage 2 (spectral norm, two-timescale LR, n_disc=2).

ABLATION PLAN
=============
Stage 1: minimal GAN                                   -> VR 0.371
Stage 2: + spectral norm + 2-timescale LR + n_disc=2  -> VR 0.511
Stage 3 (this file): + feature matching + moment matching
Stage 4: + WGAN-GP
...
USAGE
-----
    python timegan_s3.py
    python timegan_s3.py --workloads bert --epochs 50
    python timegan_s3.py --lambda-fm 0.5 --lambda-var 0.3
"""
...
OUTPUT_DIR   = Path("outputs/phase4/timegan_s3")
MODEL_DIR    = Path("models/phase4/timegan_s3")
...
TRAIN_CFG = {
    ...
    "epochs":       200,
    "lr_gen":       1e-3,
    "lr_disc":      2e-4,    # 5x slower than generator (two-timescale LR)
    "weight_decay": 1e-5,
    "patience":     25,
    "min_delta":    1e-5,
    "lambda_rec":   1.0,
    ...
    "lambda_var":   0.5,     # variance matching weight
    "lambda_auto":  0.1,     # autocorrelation matching weight
    "n_disc_steps": 2,       # discriminator updates per generator update
    "clip_stds":    2.5,
    "n_gen":        5,
}
...
class Discriminator(nn.Module):
    """
    BiLSTM sequence discriminator with spectral normalization.

    Spectral normalization (Miyato et al. 2018) constrains the Lipschitz
    constant of the discriminator by normalizing each weight matrix by its
    largest singular value. This prevents the discriminator from producing
    saturated outputs that kill the generator gradient.

    Applied via nn.utils.spectral_norm() on all Linear layers.
    No hyperparameters required — it is computed from the weights each forward pass.

    Stage 1 had no spectral norm -> discriminator could assign probability 0
    to all fakes within a few epochs, giving generator gradient = 0.
    """
    def __init__(self, n_metrics, disc_cfg):
        ...
        # spectral_norm wraps each Linear — same interface, just normalized weights
        self.classifier = nn.Sequential(
            nn.utils.spectral_norm(nn.Linear(disc_cfg["hidden_dim"] * 2, 64)),
            nn.LeakyReLU(0.2),
    ...
    def get_features(self, x):
        """
        Extract pre-classifier hidden representation for feature matching.
        x: (B, T, M) -> features: (B, hidden_dim*2)
        The hidden state encodes temporal structure and distribution shape.
        Feature matching penalizes mismatch of E[D_feat(fake)] vs E[D_feat(real)].
        """
        _, (h_n, _) = self.lstm(x)
        fwd = h_n[-2]
        bwd = h_n[-1]
...
def autocorr_lag1(x):
    """
    Per-metric lag-1 autocorrelation across the time dimension.
    x: (B, T, M) -> (M,)  mean autocorr across batch
    """
    x_c  = x - x.mean(dim=1, keepdim=True)          # center per sequence
    num  = (x_c[:, :-1, :] * x_c[:, 1:, :]).mean(dim=1)   # (B, M)
    den  = (x_c ** 2).mean(dim=1) + 1e-8              # (B, M)
    return (num / den).mean(dim=0)                    # (M,)


def train_timegan(generator, discriminator, train_dl, val_dl,
                  train_cfg, gen_cfg, device, metric_weights=None):
    """
    GAN training loop with feature matching and moment matching.

    Generator loss  = lambda_rec  * MSE(fake, target)
                    + lambda_adv  * BCE(D(fake), real_label)
                    + lambda_fm   * MSE(mean(D_feat(fake)), mean(D_feat(real)))
                    + lambda_var  * MSE(var(fake, dim=0), var(real, dim=0))
                    + lambda_auto * MSE(autocorr(fake), autocorr(real))
    ...
    # This is overwritten on the first valid (non-NaN) val_rec improvement.
    ...
    for epoch in range(1, train_cfg["epochs"] + 1):
        ...
        for x_ph, target, _, r_norm, exp_ids, ph_batch in train_dl:
            ...
            # Feature matching: match discriminator hidden features of real vs fake
            # Both calls go through the same disc (no grad for real features)
            ...
            # Moment matching: match variance and lag-1 autocorrelation of traces.
            # Use correction=0 (population variance) to handle the last batch which
            # may have B=1 (49 samples / batch_size=16 -> last batch size 1).
            # Bessel correction (default correction=1) divides by B-1 = 0 -> NaN.
            var_real  = target.var(dim=0, correction=0).detach()   # (T, M)
            var_fake  = fake.var(dim=0, correction=0)
            loss_var  = nn.functional.mse_loss(var_fake, var_real)

            ac_real   = autocorr_lag1(target).detach()  # (M,)
            ac_fake   = autocorr_lag1(fake)
            loss_auto = nn.functional.mse_loss(ac_fake, ac_real)

            loss_G = (train_cfg["lambda_rec"]  * loss_rec
                    + train_cfg["lambda_adv"]  * loss_adv
                    + train_cfg["lambda_fm"]   * loss_fm
                    + train_cfg["lambda_var"]  * loss_var
                    + train_cfg["lambda_auto"] * loss_auto)
...
def plot_comparison(workload, real_orig, syn_orig,
    ...
    fig.suptitle(
        f"{workload.upper()} - TimeGAN Stage 3 (feature/moment matching)\n"
    ...
    path = save_dir / f"{workload}_timegan_s3.png"
...
def main():
    ...
    parser.add_argument("--lambda-var",   type=float,default=TRAIN_CFG["lambda_var"])
    parser.add_argument("--lambda-auto",  type=float,default=TRAIN_CFG["lambda_auto"])
    ...
    TRAIN_CFG["lambda_var"]  = args.lambda_var
    TRAIN_CFG["lambda_auto"] = args.lambda_auto
    ...
    print("=" * 65)
    print("TimeGAN Stage 3 - Feature/Moment Matching")
    ...
    print(f"lambda_var  : {TRAIN_CFG['lambda_var']}")
    print(f"lambda_auto : {TRAIN_CFG['lambda_auto']}")
    print(f"n_disc_steps: {TRAIN_CFG['n_disc_steps']}")
    print(f"lr_gen      : {TRAIN_CFG['lr_gen']}  lr_disc: {TRAIN_CFG['lr_disc']}")
    print("Stage 3 additions: feature matching + moment matching (on top of Stage 2)")
    print("=" * 65)
    ...
    for workload in args.workloads:
        ...
        all_results.append({
            ...
            "lambda_var":           TRAIN_CFG["lambda_var"],
            "lambda_auto":          TRAIN_CFG["lambda_auto"],
            "lstm_baseline_vr":     {
    ...
    print(f"\n{'='*65}")
    print("TIMEGAN STAGE 3 SUMMARY")
    print(f"{'='*65}")
    header = f"{'Workload':<12}  {'VR S3':>8}  {'VR S2':>8}  {'VR LSTM':>8}  {'Delta S2->S3':>13}"
    print(header)
    print("-" * len(header))
    ...
    print()
    print("  If S3 > S2: feature/moment matching improved variance capture")
    print("  If S3 > LSTM: GAN now strictly better than LSTM baseline")
    print("  If S3 ~ S2: loss signal is not the bottleneck -> try WGAN-GP (S4)")

    out_path = OUTPUT_DIR / "timegan_s3_results.json"
