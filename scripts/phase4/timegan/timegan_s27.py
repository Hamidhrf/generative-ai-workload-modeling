"""
TimeGAN S27 - S21 Architecture + Pod Memory + GPU Power
========================================================
Master Thesis - Generative AI Workload Modeling
Fachhochschule Dortmund

S27 = S21 + Training pod_memory_bytes + gpu_power_watts
========================================================

KEY CHANGES FROM S21:
- pod_memory_bytes: NOW TRAINED (was dropped in S21)
- gpu_power_watts:  NOW TRAINED (was dropped in S21)
- gpu_temperature:  Still dropped (not in thesis proposal)

All workloads get +2 kept metrics compared to S21.

HYPOTHESIS:
For Whisper, adding pod_memory + gpu_power (6 metrics vs 4 in S21)
may improve pattern quality by giving the model more context about
resource state, even if these metrics have lower variance.

ARCHITECTURE: Identical to S21
- GeneratorSeg (segment-based, 120 steps × 6 phases)
- Precomputed FM targets (scale-invariant)
- WGAN-GP discriminator
- Same hyperparameters as S21

EXPECTED RESULTS:
- VR: Should stay ≥0.8 (might be slightly lower due to more metrics)
- Whisper plot quality: May improve with more context
- Other workloads: Similar to S21

USAGE
-----
    python timegan_s27.py
    python timegan_s27.py --workloads whisper
    python timegan_s27.py --adv-epochs 150

OUTPUT
------
    outputs/phase4/timegan_s27/{run_tag}/plots/
    models/phase4/timegan_s27/{run_tag}/{workload}/
    outputs/phase4/timegan_s27/{run_tag}/results.json
"""
...
# S27 DROP CONFIG: REMOVED pod_memory_bytes and gpu_power_watts
DROP_PER_WORKLOAD = {
    "bert":      {"gpu_memory_total", "gpu_memory_used",
                  "gpu_temperature"},  # 3 dropped, 7 kept
    
    "gpt2":      {"gpu_memory_total",
                  "gpu_temperature"},  # 2 dropped, 8 kept
    
    "resnet152": {"gpu_memory_total", "gpu_memory_used",
                  "gpu_temperature"},  # 3 dropped, 7 kept
    
    "whisper":   {"gpu_memory_total", "gpu_memory_used",
                  "pod_throughput", "gpu_temperature"},  # 4 dropped, 6 kept
    
    "yolo":      {"gpu_memory_total", "gpu_memory_used",
                  "gpu_temperature"},  # 3 dropped, 7 kept
}

# S27: segment-based constants (identical to S21)
...
# S27 WORKLOAD_OVERRIDES (identical to S21)
WORKLOAD_OVERRIDES = {
...
REFERENCE_VR = {
    "bert":      {"s21": 0.672, "s19": 1.226},
    "gpt2":      {"s21": 0.996, "s19": 1.011},
    "resnet152": {"s21": 0.960, "s19": 1.024},
    "whisper":   {"s21": 1.722, "s19": 0.788},  # S21 Whisper VR from search
    "yolo":      {"s21": 0.661, "s19": 1.228},
}

# Copy all helper functions from S21 (identical)
# ------------------------------------------------------------------
...
# Evaluate + plotting functions (identical to S21 - omitted for brevity)
# Copy from S21: evaluate(), build_phase_sequence(), plot_comparison()
...
def main():
    parser = argparse.ArgumentParser(description="TimeGAN S27")
    ...
    run_tag  = (f"s27_seg_vr{int(args.var_reg * 10):02d}"
                f"_fm{int(args.fm_stat * 10):02d}"
                f"_ae{args.adv_epochs}")
    out_dir  = Path(f"outputs/phase4/timegan_s27/{run_tag}")
    model_dir = Path(f"models/phase4/timegan_s27/{run_tag}")
    ...
    print("=" * 70)
    print("TimeGAN S27 - S21 + Pod Memory + GPU Power")
    print("=" * 70)
    print(f"Device          : {device}")
    print(f"Architecture    : GeneratorSeg (S21)")
    print(f"NEW KEPT METRICS: pod_memory_bytes, gpu_power_watts")
    ...
    for workload in args.workloads:
        ...
        print(f"  S21 had: {n_metrics - 2} metrics (this is +2)")
        ...
        print(f"    var_ratio     = {vr_mean:.4f}  "
              f"(s21={ref_vr.get('s21','?'):.4f}  s19={ref_vr.get('s19','?'):.4f})")
    ...
    # Summary
    print(f"\n{'=' * 72}")
    print(f"S27 SUMMARY  ({run_tag})")
    print(f"{'=' * 72}")
    print(f"  {'Workload':<12} {'Metrics':>8} {'VR S27':>8} {'VR S21':>8} {'VR S19':>8}")
    print(f"  {'-' * 72}")
    vr_means = []
