"""
Adapted from timegan_s36_tier2.py (itself adapted from
timegan_s36_tier3_sweep.py) for the Frame C hyperparameter
ablation on Tier 2 MIG data. Adds --workload/--vr/--fm CLI
overrides so a single workload can be trained with a
one-off (lambda_var_reg, lambda_fm_stat) point instead of
the frozen S27_HYPERPARAMS entry. Output directory changed
to models/phase4/timegan_s36_tier2_sweep/ and
outputs/phase4/timegan_s36_tier2_sweep/ so sweep runs never
collide with Step 4's frozen-hyperparam checkpoints/summary.
Everything else (architecture, seeds, warmup/adversarial
epoch counts, data paths) is untouched.
...
def main():
    ...
    parser.add_argument("--workload", type=str, default=None,
                        help="Single workload to train with --vr/--fm override "
                             "(restricts run to just this workload)")
    parser.add_argument("--vr", type=float, default=None,
                        help="Override lambda_var_reg for --workload")
    parser.add_argument("--fm", type=float, default=None,
                        help="Override lambda_fm_stat for --workload")
    ...
    if args.workload is not None:
        assert args.vr is not None and args.fm is not None, \
            "--workload requires both --vr and --fm"
        run_workloads = [args.workload]
        hyperparams_override = dict(S27_HYPERPARAMS[args.workload])
        hyperparams_override["lambda_var_reg"] = args.vr
        hyperparams_override["lambda_fm_stat"] = args.fm
    ...
    if hyperparams_override is not None:
        print(f"Sweep override: {hyperparams_override}")
    ...
    for wl in run_workloads:
        ...
        hyperparams = hyperparams_override if hyperparams_override is not None else S27_HYPERPARAMS[wl]
        ...
        model_dir = Path(f"models/phase4/timegan_s36_tier2_sweep/{tag}")
