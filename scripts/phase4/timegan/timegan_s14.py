#!/usr/bin/env python3
"""
TimeGAN S14 - Encoder-Free Generator (GeneratorB)
===================================================
Master Thesis - Generative AI Workload Modeling
Fachhochschule Dortmund

ROOT CAUSE OF ALL S7-S12 FAILURES
====================================
Every stage S7-S12 used GeneratorA which has an encoder:

    TRAIN:    real_trace -> LSTM_encoder -> z -> LSTM_decoder -> fake_trace
    GENERATE: random_z              ----> LSTM_decoder -> trace

During training the encoder compresses a real trace into z. The decoder
learns to reconstruct from z. Warmup val_rec reaches 0.001 - this works.
But at GENERATE time z is replaced with torch.randn(). The latent space was
never regularised to be sampled from (no VAE KL term, no contractive loss).
A random z lands far outside the distribution of encoded real-trace z values.
The decoder has never seen these z values and outputs garbage.

The generator's best strategy under adversarial training becomes:
output per-phase means (flat dashes) - the discriminator cannot detect
these as fake because the means are correct, and the reconstruction loss
is low. This is the flat synthetic trace pattern visible in every S7-S12
BERT, ResNet152, YOLO plot.

All auxiliary losses (var_reg, fm_stat, feature matching) cannot fix this
because they are fighting outputs produced by a broken generation path.

S14 FIX: REMOVE THE ENCODER COMPLETELY
=========================================
GeneratorB maps (z, r_norm, phase_ids) directly to a trace.
TRAIN and GENERATE are identical - always sample z ~ N(0, latent_dim).

Architecture:

    z ~ N(0, latent_dim)          # sampled at train AND generate time
    r_emb = r_embed(r_norm)       # scalar r_norm -> dense embedding
    h0, c0 = MLP(cat[z, r_emb])  # initial hidden state from z and r
    dec_in[t] = cat[z, r_emb, ph_emb[t]]   # z injected every timestep
    LSTM_decoder(dec_in, h0, c0)  # unrolls T steps
    output = sigmoid(fc(lstm_out))

Why this works:
  - z controls stochastic variation (different z -> different trace shape)
  - r_norm controls the systematic contention effect
  - phase_ids control which business-day phase is active at each timestep
  - No encoder means no train/test mismatch
  - The discriminator sees z-driven diversity and has a real job to do

WARMUP STRATEGY CHANGE
========================
Without an encoder there is no reconstruction loss.
Warmup now uses only fm_stat (per-replica mean+std matching) and var_reg
(log-ratio variance) for 20 epochs. This forces the generator to produce
outputs with correct statistical properties per replica count before the
adversarial term is added, giving the generator a head start and preventing
the discriminator from winning immediately.

Epoch 1-20  (statistical warmup):
    loss_G = lambda_fm_stat * fm_stat_loss + lambda_var_reg * var_reg_loss

Epoch 21+   (adversarial):
    loss_G = adv + lambda_fm_stat * fm_stat + lambda_var_reg * var_reg
                 + lambda_smooth * smooth

HYPERPARAMETER CHANGES FROM S12
==================================
Architecture:    GeneratorA (encoder+decoder) -> GeneratorB (decoder only)
lambda_rec_post: 0.3 -> N/A (no reconstruction loss)
lambda_var_reg:  1.0 -> 0.3 (log-ratio, inherently much stronger)
var_loss_type:   huber -> log_ratio
lambda_fm_stat:  0.5 (unchanged)
lambda_smooth:   per-workload (unchanged)
enc_dropout:     removed (no encoder)
Warmup:          reconstruction -> statistical (fm_stat + var_reg only)

ALL ELSE UNCHANGED
====================
Discriminator:   conditional biLSTM h=32 l=1 + r_norm
n_disc_steps:    per-workload (bert/gpt2/resnet152/yolo=2, whisper=3)
lambda_smooth:   bert/gpt2/resnet152/yolo=0.1, whisper=0.3
...
S8             1.678  honest best VR, jump ratios bad (Whisper 42x)
S9             0.878  most balanced, ResNet152 0.324
S10            1.937  inflated - psi_cpu capped 4/5
S11            1.978  inflated + flat synthetics
S12            2.053  Whisper improved; psi_cpu still capped 3/5
S13            TBD    (log-ratio var loss, same broken encoder arch)
S14            THIS   encoder-free GeneratorB, train/test mismatch fixed

USAGE
-----
    python timegan_s14.py                          # all workloads
    python timegan_s14.py --workloads bert yolo    # subset
    python timegan_s14.py --var-reg 0.3            # log-ratio var strength
    python timegan_s14.py --fm-stat 0.5            # per-replica fm strength
    python timegan_s14.py --adv-epochs 150

OUTPUT
------
    outputs/phase4/timegan_s14/{run_tag}/plots/
    models/phase4/timegan_s14/{run_tag}/{workload}/
    outputs/phase4/timegan_s14/{run_tag}/results.json

    run_tag = s14_vr{var_reg*10}_fm{fm_stat*10}_ae{adv_epochs}
    e.g.      s14_vr03_fm05_ae150
"""
...
REFERENCE_VR = {
    "bert":      {"lstm": 0.594, "s8": 2.645, "s9": 1.213, "s12": 2.240},
    "gpt2":      {"lstm": 0.676, "s8": 2.315, "s9": 0.664, "s12": 2.710},
    "resnet152": {"lstm": 0.607, "s8": 1.055, "s9": 0.324, "s12": 2.490},
    "whisper":   {"lstm": 0.748, "s8": 1.042, "s9": 0.743, "s12": 2.068},
    "yolo":      {"lstm": 0.438, "s8": 1.334, "s9": 1.446, "s12": 0.758},
}

REFERENCE_JUMP = {
    "bert":      {"s8":  3.9, "s9":  8.7, "s12": 0.23},
    "gpt2":      {"s8":  3.0, "s9":  7.2, "s12": 3.18},
    "resnet152": {"s8":  6.5, "s9": 17.1, "s12": 0.16},
    "whisper":   {"s8": 42.1, "s9": 26.2, "s12": 0.66},
    "yolo":      {"s8": 10.6, "s9": 11.2, "s12": 0.32},
}
...
def replica_interpolation(proc_traces, zmph, trace_means,
    ...
    for i in range(len(unique_r_sorted) - 1):
        ...
        if n == 0:
            continue
        for k in range(n):
            new_zm.append(0.5 * zmph[idx_lo[k]]          + 0.5 * zmph[idx_hi[k]])
            new_proc.append(0.5 * proc_traces[idx_lo[k]]  + 0.5 * proc_traces[idx_hi[k]])
            new_means.append(0.5 * trace_means[idx_lo[k]] + 0.5 * trace_means[idx_hi[k]])
            new_r.append(r_mid)
            new_eids.append(-1)
...
# ------------------------------------------------------------------
# Dataset
# S14 note: the dataset still provides inp (zeromean trace) but it is
# used only for computing fm_stat targets during warmup. The generator
# does not receive inp - it only receives r_norm and phase_ids.
# ------------------------------------------------------------------
...
def evaluate(generator, zm_traces, trace_means, raw_kept,
...
def main():
    ...
    run_tag    = (f"s14_vr{int(args.var_reg * 10):02d}"
                  f"_fm{int(args.fm_stat   * 10):02d}"
                  f"_ae{args.adv_epochs}{interp_tag}")

    out_dir   = Path(f"outputs/phase4/timegan_s14/{run_tag}")
    model_dir = Path(f"models/phase4/timegan_s14/{run_tag}")
    out_dir.mkdir(parents=True, exist_ok=True)
    model_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 70)
    print("TimeGAN S14 - Encoder-Free Generator (GeneratorB)")
    print("=" * 70)
    ...
    for workload in args.workloads:
        ...
        (real_orig, syn_orig, real_r_arr, syn_r_arr,
         vr, vr_mean, ac_diff, jump_ratio) = evaluate(
            generator, zm_with_phase, trace_means, raw_kept,
            replica_counts, phase_seq, train_idx_orig,
        ...
        print(f"    var_ratio     = {vr_mean:.4f}  "
              f"(s12={ref_vr.get('s12','?'):.4f}  s9={ref_vr.get('s9','?'):.4f}  "
              f"lstm={ref_vr.get('lstm','?'):.4f})")
        print(f"    autocorr_diff = {ac_diff:.4f}")
        print(f"    jump_ratio    = {jump_ratio:.3f}x  "
              f"(s12={ref_jump.get('s12','?'):.3f}x  s9={ref_jump.get('s9','?'):.1f}x)")
