"""
TimeGAN S28 - Complete Training (All 8 Meaningful Metrics)
===========================================================
Master Thesis - Generative AI Workload Modeling
Fachhochschule Dortmund

S28 = S21 Architecture + Train ALL 8 Meaningful Metrics
========================================================

PROGRESSION:
- S21: Dropped pod_memory_bytes, gpu_power_watts (5 metrics for most workloads)
- S27: Added pod_memory_bytes, gpu_power_watts (6-8 metrics, variable per workload)
- S28: Train ALL 8 metrics for ALL workloads (complete consistency)

S28 TRAINS (8 metrics for ALL workloads):
------------------------------------------
1. pod_cpu_usage       (idx 0) - Core compute resource
2. pod_memory_bytes    (idx 1) - Memory footprint
3. pod_psi_cpu         (idx 2) - CPU pressure/contention
4. pod_latency_avg     (idx 3) - QoS metric (response time)
5. pod_throughput      (idx 4) - QoS metric (requests/sec)
6. gpu_utilization     (idx 5) - GPU compute usage
7. gpu_memory_used     (idx 6) - GPU memory allocation
8. gpu_power_watts     (idx 8) - Power consumption

S28 DROPS (2 metrics, ALL workloads):
--------------------------------------
- gpu_memory_total (idx 7) - Constant 15360 MB (model capacity, not workload behavior)
- gpu_temperature  (idx 9) - High correlation with gpu_utilization (R²>0.9)

SCIENTIFIC JUSTIFICATION:
-------------------------
S28 provides the most rigorous evaluation by training all meaningful resource
metrics directly. If S28 achieves VR>0.8, it demonstrates the model can capture
all aspects of workload behavior without reconstruction. If S28 shows lower VR
than S27, it suggests gpu_memory_used has low variance and reconstruction is
scientifically acceptable.

Dropped metrics justified:
- gpu_memory_total: Hardware spec, not workload-dependent
- gpu_temperature: Redundant (linear function of utilization + ambient temp)

ARCHITECTURE: Identical to S21/S27
-----------------------------------
- GeneratorSeg (segment-based, 120 steps x 6 phases)
- Precomputed FM targets (scale-invariant)
- WGAN-GP discriminator
- Same hyperparameters as S21/S27

EXPECTED RESULTS:
-----------------
- VR: Target >=0.8 (might be slightly lower than S27 due to more metrics)
- If VR<0.8: Indicates gpu_memory_used has inherent low variance
- If VR>=0.8: Demonstrates complete workload modeling capability

USAGE
-----
    python timegan_s28.py
    python timegan_s28.py --workloads whisper
    python timegan_s28.py --adv-epochs 150

OUTPUT
------
    outputs/phase4/timegan_s28/{run_tag}/plots/
    models/phase4/timegan_s28/{run_tag}/{workload}/
    outputs/phase4/timegan_s28/{run_tag}/results.json
"""
...
# S28 DROP CONFIG: ONLY gpu_memory_total and gpu_temperature
# ALL workloads train SAME 8 metrics
DROP_PER_WORKLOAD = {
    "bert":      {"gpu_memory_total", "gpu_temperature"},  # 8 kept
    "gpt2":      {"gpu_memory_total", "gpu_temperature"},  # 8 kept
    "resnet152": {"gpu_memory_total", "gpu_temperature"},  # 8 kept
    "whisper":   {"gpu_memory_total", "gpu_temperature"},  # 8 kept
    "yolo":      {"gpu_memory_total", "gpu_temperature"},  # 8 kept
}

# S28: segment-based constants (identical to S21/S27)
...
# S28 WORKLOAD_OVERRIDES (identical to S21/S27)
WORKLOAD_OVERRIDES = {
...
REFERENCE_VR = {
    "bert":      {"s21": 0.672, "s27": 0.886},
    "gpt2":      {"s21": 0.996, "s27": 0.946},
    "resnet152": {"s21": 0.960, "s27": 0.851},
    "whisper":   {"s21": 1.722, "s27": 1.630},
    "yolo":      {"s21": 0.661, "s27": 0.997},
}

# Copy all helper functions from S27 (identical)
# ------------------------------------------------------------------
...
def validate_replica_counts(replica_counts, n_pods, metadata, workload):
    ...
    print(f"  [WARN] replica_counts mismatch; rebuilding from metadata")
...
# ------------------------------------------------------------------
# GeneratorSeg (identical to S21/S27)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Discriminator (identical to S21/S27)
# ------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(description="TimeGAN S28 - Complete Training")
    ...
    run_tag  = (f"s28_seg_vr{int(args.var_reg * 10):02d}"
                f"_fm{int(args.fm_stat * 10):02d}"
                f"_ae{args.adv_epochs}")
    out_dir  = Path(f"outputs/phase4/timegan_s28/{run_tag}")
    model_dir = Path(f"models/phase4/timegan_s28/{run_tag}")
    out_dir.mkdir(parents=True, exist_ok=True)

    boundaries = DEFAULT_PHASE_BOUNDARIES
    full_boundaries = boundaries + [FULL_SEQ_LEN]

    print("=" * 70)
    print("TimeGAN S28 - Complete Training (All 8 Meaningful Metrics)")
    print("=" * 70)
    print(f"Device          : {device}")
    print(f"Architecture    : GeneratorSeg (S21)")
    print(f"ALL WORKLOADS   : 8 metrics (SAME for consistency)")
    print(f"Trained metrics : pod_cpu, pod_memory, pod_psi_cpu, latency,")
    print(f"                  throughput, gpu_util, gpu_memory_used, gpu_power")
    print(f"Dropped metrics : gpu_memory_total (constant), gpu_temperature")
    print(f"Segment length  : {SEGMENT_LEN} x {N_PHASES} phases = {FULL_SEQ_LEN}")
    print(f"Warmup          : {args.stat_warmup} epochs")
    print(f"Var regression  : lambda={args.var_reg} (log-ratio)")
    print(f"Segment FM      : lambda_fm_stat={args.fm_stat}")
    ...
    for workload in args.workloads:
        ...
        if n_metrics != 8:
            print(f"  WARNING: Expected 8 metrics, got {n_metrics}")
        ...
        print(f"  Segments: train={n_train_segs}, val={n_val_segs}")
        ...
        n_groups = len(fm_targets)
        print(f"  FM targets: {n_groups} (phase,r) groups precomputed")
        ...
        print(f"    var_ratio     = {vr_mean:.4f}  "
              f"(s27={ref_vr.get('s27','?'):.4f}  s21={ref_vr.get('s21','?'):.4f})")
    ...
    # Summary
    print(f"\n{'=' * 72}")
    print(f"S28 SUMMARY  ({run_tag})")
    print(f"{'=' * 72}")
    print(f"  {'Workload':<12} {'Metrics':>8} {'VR S28':>8} {'VR S27':>8} {'VR S21':>8}")
    print(f"  {'-' * 72}")
    vr_means = []
    for r in all_results:
        ...
        print(f"  {r['workload']:<12} "
              ...
              f"{ref_vr.get('s27', 0):>8.4f} "
              f"{ref_vr.get('s21', 0):>8.4f}")
    print(f"  {'-' * 72}")
    print(f"  {'MEAN':<12} {'':<8} {sum(vr_means)/len(vr_means):>8.4f}")
    print(f"\nTarget: mean VR > 0.8")
    print(f"\nDECISION:")
    mean_vr = sum(vr_means)/len(vr_means)
    if mean_vr >= 0.8:
        print(f"  ✓ S28 VR={mean_vr:.4f} >= 0.8 → USE S28 (best scientific rigor)")
    else:
        print(f"  ✗ S28 VR={mean_vr:.4f} < 0.8 → USE S27 + reconstruct gpu_memory_used")
