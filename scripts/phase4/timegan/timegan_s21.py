...
USAGE
-----
    python timegan_s21.py
    python timegan_s21.py --workloads whisper yolo
    python timegan_s21.py --var-reg 0.3 --fm-stat 1.0 --adv-epochs 150

OUTPUT
------
    outputs/phase4/timegan_s21/{run_tag}/plots/
    models/phase4/timegan_s21/{run_tag}/{workload}/
    outputs/phase4/timegan_s21/{run_tag}/results.json

    run_tag = s21_seg_vr{var_reg*10}_fm{fm_stat*10}_ae{adv_epochs}
"""
...
REFERENCE_VR = {
    "bert":      {"lstm": 0.594, "s9": 1.213, "s19": 1.226, "s20": 0.621},
    "gpt2":      {"lstm": 0.676, "s9": 0.664, "s19": 1.011, "s20": 1.253},
    "resnet152": {"lstm": 0.607, "s9": 0.324, "s19": 1.024, "s20": 0.752},
    "whisper":   {"lstm": 0.748, "s9": 0.743, "s19": 0.788, "s20": 0.661},
    "yolo":      {"lstm": 0.438, "s9": 1.446, "s19": 1.228, "s20": 0.737},
}

REFERENCE_JUMP = {
    "bert":      {"s8":  3.9, "s9":  8.7, "s19": 0.083, "s20": 2.104},
    "gpt2":      {"s8":  3.0, "s9":  7.2, "s19": 0.158, "s20": 0.128},
    "resnet152": {"s8":  6.5, "s9": 17.1, "s19": 0.095, "s20": 0.697},
    "whisper":   {"s8": 42.1, "s9": 26.2, "s19": 0.285, "s20": 2.068},
    "yolo":      {"s8": 10.6, "s9": 11.2, "s19": 0.122, "s20": 6.617},
}
...
def main():
    ...
    run_tag  = (f"s21_seg_vr{int(args.var_reg * 10):02d}"
                f"_fm{int(args.fm_stat * 10):02d}"
                f"_ae{args.adv_epochs}")
    out_dir  = Path(f"outputs/phase4/timegan_s21/{run_tag}")
    model_dir = Path(f"models/phase4/timegan_s21/{run_tag}")
