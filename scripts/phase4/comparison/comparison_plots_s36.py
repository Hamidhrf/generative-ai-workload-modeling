"""
S36 Comparison Plots
====================
Visualize real vs S36 synthetic traces for all 5 workloads.

S36 = S34 architecture + spectral normalization on discriminator
Uses original (non-augmented) dataset.

Usage:
  python comparison_plots_s36.py --replica-counts 1 5 10
  python comparison_plots_s36.py --workloads bert gpt2
"""
...
# 7 plot metrics (S36 trained metrics - dropped gpu_memory_used, gpu_memory_total, gpu_temperature)
PLOT_METRICS = [
...
# S36: All 5 workloads have same 7 metrics (dropped gpu_memory_total, gpu_memory_used, gpu_temperature)
S36_TRAINED_INDICES = [0, 1, 2, 3, 4, 5, 8]  # Indices in 10-metric array
S36_TRAINED_NAMES = ['pod_cpu_usage', 'pod_memory_bytes', 'pod_psi_cpu',
...
class GeneratorSeg(nn.Module):
    """S36 generator (identical architecture to S34 - spectral norm only on discriminator)."""
...
def load_normalization_params(workload, data_dir='data/processed/phase4/unified'):
    """Load normalization parameters (original dataset, not augmented)."""
...
def denormalize_trace(trace_norm, norm_params, metric_names):
    """Denormalize using minmax normalization."""
...
def load_generator(workload, device='cuda'):
    ...
    n_metrics = 7  # All S36 workloads have 7 metrics
...
def generate_trace_s36(workload, replica_count, device='cuda'):
    ...
    trace_denorm = denormalize_trace(trace_norm, norm_params, S36_TRAINED_NAMES)

    # Build full 10-metric trace (with zeros for dropped metrics)
    trace_full = np.zeros((715, 10))

    for i, idx in enumerate(S36_TRAINED_INDICES):
...
def get_real_trace_sample(workload, replica_count):
    ...
    trace_norm_7 = trace_norm[:, S36_TRAINED_INDICES]

    # Denormalize
    trace_denorm = denormalize_trace(trace_norm_7, norm_params, S36_TRAINED_NAMES)

    # Build full 10-metric trace
    trace_full = np.zeros((715, 10))
    for i, midx in enumerate(S36_TRAINED_INDICES):
        trace_full[:, midx] = trace_denorm[:, i]

    return trace_full


def create_comparison_plot(workload, replica_count, output_dir):
    """Create comparison plot for S36."""

    print(f"  {workload.upper()} r={replica_count}...", end='', flush=True)

    try:
        real_trace = get_real_trace_sample(workload, replica_count)
        synth_trace = generate_trace_s36(workload, replica_count)
    ...
    for plot_idx, metric_info in enumerate(PLOT_METRICS):
        ...
        ax.plot(time_steps, synth_vals, color='darkorange', lw=1.2, alpha=0.7, label='S36')
    ...
    fig.suptitle(f"S36 (Spectral Norm): {workload.upper()} (r={replica_count})",
                 fontsize=14, fontweight='bold', y=0.995)

    output_path = output_dir / f"{workload}_r{replica_count}_s36.png"
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='outputs/phase4/comparison_plots/s36')
    ...
    print("=" * 80)
    print("S36 COMPARISON PLOTS (Spectral Normalization)")
    print("=" * 80)
    print(f"Workloads: {args.workloads}")
    print(f"Replica counts: {args.replica_counts}")
    print(f"Metrics: 7 (same as S34)")
    print(f"Dataset: Original (non-augmented) - combined_dataset.npz")
