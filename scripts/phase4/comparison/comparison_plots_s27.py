"""
S27 Comparison Plots - FINAL CORRECTED
Uses workload-specific normalization parameters from JSON files

Usage:
  python comparison_plots_s27_FINAL.py --replica-counts 1 5 10
"""
...
def load_generator(workload, device='cuda'):
    ...
    if not ckpt_path.exists():
        raise FileNotFoundError(f"S27 checkpoint: {ckpt_path}")
...
def generate_trace_s27(workload, replica_count, device='cuda'):
    ...
    for idx in S27_RECONSTRUCT[workload]:
...
def create_comparison_plot(workload, replica_count, output_dir):
    ...
    synth_trace = generate_trace_s27(workload, replica_count)
    ...
    for plot_idx, metric_info in enumerate(PLOT_METRICS):
        ...
        ax.plot(time_steps, synth_vals, color='darkorange', lw=1.2, alpha=0.7, label='S27')
    ...
    fig.suptitle(f"S27: {workload.upper()} (r={replica_count})", 
                 fontsize=14, fontweight='bold', y=0.995)
    
    output_path = output_dir / f"{workload}_r{replica_count}_s27.png"
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='outputs/phase4/comparison_plots/s27')
    ...
    print("=" * 70)
    print("S27 Comparison Plots - FINAL (Workload-Specific Normalization)")
    print("=" * 70)
