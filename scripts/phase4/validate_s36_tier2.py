"""
Adapted from validate_s36_tier3.py for H100 Tier 2 MIG data. Changed:
the postprocess_s36_tier2 import (so synthetic generation
uses Tier 2 models/data, not just this file's own data_dir
defaults -- generate_postprocessed_traces() is called without
a data_dir override at every call site in this file, so it
falls back to whatever postprocess_s36(_tier2) hardcodes),
...
-- frozen per the S36 Tier 2 retrain plan. NOTE: REPLICA_COUNTS,
REPLICA_COUNTS_PER_WORKLOAD, and the extrapolation-zone plot
boundary are Tier-2-specific (r=1..7 data range), not copied
verbatim from Tier 3's r=1..10 -- Tier 2's real data tops out
at r=7, so the Tier 3 constants would look up nonexistent
replica counts and crash (confirmed: KeyError on r=8 in
plot_scaling_curves before this fix was applied).
...
from postprocess_s36_tier2 import (
...
REPLICA_COUNTS = [1, 2, 3, 4, 5, 6, 7]

# Extrapolation replica counts (no real data - exploratory)
EXTRAPOLATION_COUNTS = [15, 20, 30, 50]

# Tier 2 has complete r=1..7 data for all 5 workloads (no gaps,
# unlike Tier 3's missing whisper r=8) -- same list for all 5.
REPLICA_COUNTS_PER_WORKLOAD = {
    'bert': [1, 2, 3, 4, 5, 6, 7],
    'gpt2': [1, 2, 3, 4, 5, 6, 7],
    'resnet152': [1, 2, 3, 4, 5, 6, 7],
    'whisper': [1, 2, 3, 4, 5, 6, 7],
    'yolo': [1, 2, 3, 4, 5, 6, 7],
}
...
def get_real_stats(data_dir='data/processed/tier2/unified'):
    """
...
def plot_scaling_curves(real_stats, synth_stats, workloads, output_dir):
    ...
    for wname in workloads:
        ...
        for pi, m in enumerate(ANALYSIS_METRICS):
            ...
            if extrap_replicas:
                ax.axvspan(7.5, max(extrap_replicas) + 2,
                           alpha=0.08, color='red', label='Extrapolation zone')
                ax.axvline(7.5, color='red', ls=':', alpha=0.4, lw=1)
...
def plot_combined_scaling(real_stats, synth_stats, workloads, output_dir):
    ...
    for pi, m in enumerate(ANALYSIS_METRICS):
        ...
        if max_extrap > 10:
            ax.axvspan(7.5, max_extrap + 2, alpha=0.08, color='red')
            ax.axvline(7.5, color='red', ls=':', alpha=0.4, lw=1)
...
def main():
    ...
    parser.add_argument('--output-dir', type=str,
                        default='outputs/phase4/validation/s36_tier2')
