"""
S33 Evaluation with Boundary Smoothing
=======================================
Evaluate S33 models (GPU-bound only) with automatic boundary smoothing.

S33 = S27 architecture + GPU-bound workloads only (no Whisper)

Usage:
    python eval_s33.py
    python eval_s33.py --workloads bert gpt2
    python eval_s33.py --window_size 3  # Less aggressive smoothing
"""
...
S33_WORKLOADS = ["bert", "gpt2", "resnet152", "yolo"]  # GPU-bound only
...
def evaluate_workload(workload_name, device, window_size=5, n_gen=5):
    ...
    # Load S33 model
    import glob
    model_dirs = glob.glob(f"models/phase4/timegan_s33/s33_{workload_name}_*")
    
    if not model_dirs:
        print(f"ERROR: No S33 model found for {workload_name}")
        return None
    ...
    pass_raw = (mean_vr_raw > 0.8)
    pass_smooth = (mean_vr_smooth > 0.8)
    ...
    return {
        ...
        "pass_raw": bool(pass_raw),
        "pass_smooth": bool(pass_smooth),
...
def main():
    parser = argparse.ArgumentParser(description="S33 Evaluation with Smoothing")
    parser.add_argument("--workloads", nargs='+', type=str,
                        default=S33_WORKLOADS,
    ...
    print("="*80)
    print("S33 EVALUATION (GPU-BOUND + SMOOTHING)")
    print("="*80)
    ...
    with open(out_dir / "s33_eval_results.json", "w") as f:
        json.dump({
            ...
            "summary": {
                ...
                "pass_count": sum(1 for r in results.values() if r["pass_smooth"]),
                "total": len(results),
            }
        }, f, indent=2)
    ...
    print(f"\n{'Workload':<15} {'VR S33':<10} {'VR S27':<10} {'Status':<10}")
    print("-"*80)
    ...
    summary = {
        "mean_vr": np.mean([r["vr_smooth"] for r in results.values()]),
        "pass_count": sum(1 for r in results.values() if r["pass_smooth"]),
    }
    ...
    print("RESULTS SAVED")
    print("="*80)
    print(f"JSON: {out_dir / 's33_eval_results.json'}")
