"""
S38 Full Evaluation - Compare S38 vs S36
=========================================
Mirrors eval_s37.py protocol exactly. The only differences from eval_s38.py
in the project are: (1) consistent file naming as "_full" so we don't
overwrite any older s38_eval_results.json that may exist, (2) explicit JSON
output to outputs/phase4/timegan_s38/s38_eval_results.json so the schema
matches s36_eval_results.json and s37_eval_results.json.

S38 = S36 architecture + multi-layer feature matching in training.
The generator is architecturally identical to S36, so we use the same
GeneratorSeg class.

Usage:
    python eval_s38_full.py
    python eval_s38_full.py --workloads bert gpt2
    python eval_s38_full.py --window_size 3
"""
...
# S38: Same 7 metrics as S34/S36/S37
DROP_PER_WORKLOAD = {
...
class GeneratorSeg(nn.Module):
    """S27/S34/S36/S37/S38 generator (architecturally identical across these stages)."""
...
def evaluate_workload(workload_name, device, window_size=5, n_gen=5):
    ...
    # S38 model paths (same naming convention as S36/S37)
    model_paths = {
...
def main():
    ...
    print(f"Architecture: S36 + Multi-Layer Feature Matching")
    print("="*80)
    ...
    with open(out_dir / "s38_eval_results.json", "w") as f:
        json.dump({
    ...
    if s36_results:
        print(f"\n{'Workload':<15} {'S38 VR':<10} {'S36 VR':<10} {'Change':<12} {'Direction'}")
        print("-"*80)
        ...
        for wl in args.workloads:
            if wl not in s38_results:
                continue
            s38_vr = s38_results[wl]["vr_smooth"]
            s36_vr = s36_results["workloads"].get(wl, {}).get("vr_smooth", 0)

            s38_dist = abs(s38_vr - 1.0)
            s36_dist = abs(s36_vr - 1.0)
            change = s38_dist - s36_dist
            ...
            print(f"{wl:<15} {s38_vr:<10.3f} {s36_vr:<10.3f} {change:+.3f} ({direction:<8}) {status}")

        s38_mean = float(np.mean([r["vr_smooth"] for r in s38_results.values()]))
        s36_mean = s36_results.get("summary", {}).get("mean_vr_smooth", 0)

        s38_mean_dist = abs(s38_mean - 1.0)
        s36_mean_dist = abs(s36_mean - 1.0)
        mean_change = s38_mean_dist - s36_mean_dist

        print("-"*80)
        print(f"{'MEAN':<15} {s38_mean:<10.3f} {s36_mean:<10.3f} {mean_change:+.3f}")
        print(f"\nMean VR distance from 1.0:")
        print(f"  S36: {s36_mean_dist:.3f}")
        print(f"  S38: {s38_mean_dist:.3f}")
        print(f"  Change: {mean_change:+.3f}")
        print(f"\nWorkload outcomes: improved={improvements}/5, degraded={degradations}/5")

    print(f"\nResults saved: {out_dir / 's38_eval_results.json'}")
    print("="*80)
