"""
Adapted from eval_s36_tier2.py (itself adapted from
eval_s36_tier3_sweep.py) for the Frame C hyperparameter
ablation on Tier 2 MIG data. Adds --workload/--model-dir/
--variant-tag CLI so a single (workload, variant) checkpoint
from models/phase4/timegan_s36_tier2_sweep/ can be evaluated
directly, bypassing the frozen model_paths dict. Writes its
own per-variant result file
(outputs/phase4/timegan_s36_tier2_sweep/eval_<workload>_<variant_tag>.json)
instead of overwriting the shared s36_eval_results.json. VR
computation, smoothing, and thresholds are byte-identical to
eval_s36_tier2.py.

S36 Evaluation - Compare S36 vs S34
====================================
Evaluate S36 models (spectral normalization) and compare against S34 baseline.

ABLATION STUDY: Does spectral normalization improve VR?
- S34 baseline: Mean VR = 1.205
- S36: S34 + Spectral Norm in discriminator

Usage:
    python eval_s36.py
    python eval_s36.py --workloads bert gpt2
    python eval_s36.py --window_size 3  # Less aggressive smoothing
"""
...
def evaluate_workload(workload_name, device, window_size=5, n_gen=5, model_path_override=None):
    ...
    # Load S36 model
    model_paths = {
        "bert": "models/phase4/timegan_s36_tier2/s36_bert_vr05_fm15/generator.pt",
        "gpt2": "models/phase4/timegan_s36_tier2/s36_gpt2_vr03_fm20/generator.pt",
        "resnet152": "models/phase4/timegan_s36_tier2/s36_resnet152_vr04_fm10/generator.pt",
        "whisper": "models/phase4/timegan_s36_tier2/s36_whisper_vr05_fm05/generator.pt",
        "yolo": "models/phase4/timegan_s36_tier2/s36_yolo_vr03_fm12/generator.pt",
    }
    
    model_path = Path(model_path_override) if model_path_override is not None else Path(model_paths[workload_name])
    if not model_path.exists():
        print(f"ERROR: No S36 model found for {workload_name}")
...
def load_s34_results():
    """Load S34 baseline results for comparison."""
    s34_path = Path("outputs/phase4/timegan_s34/s34_eval_results.json")
    if not s34_path.exists():
        print(f"\nWARNING: S34 results not found at {s34_path}")
        print("Run eval_s34.py first to generate baseline results.")
        return None
    
    with open(s34_path) as f:
        return json.load(f)


def main():
    parser = argparse.ArgumentParser(description="S36 Evaluation - Compare vs S34")
    ...
    parser.add_argument("--workload", type=str, default=None,
                        help="Single workload to evaluate against --model-dir "
                             "(restricts run to just this workload)")
    parser.add_argument("--model-dir", type=str, default=None,
                        help="Directory containing generator.pt for a sweep variant "
                             "(e.g. models/phase4/timegan_s36_tier2_sweep/s36_bert_vr03_fm15)")
    parser.add_argument("--variant-tag", type=str, default=None,
                        help="Label for this sweep variant, used in the output filename "
                             "(e.g. vr_low)")
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    sweep_mode = args.model_dir is not None
    if sweep_mode:
        assert args.workload is not None and args.variant_tag is not None, \
            "--model-dir requires both --workload and --variant-tag"
        run_workloads = [args.workload]
        model_path_override = str(Path(args.model_dir) / "generator.pt")
    else:
        run_workloads = args.workloads
        model_path_override = None

    print("="*80)
    print("S36 EVALUATION - ABLATION STUDY: SPECTRAL NORMALIZATION")
    print("="*80)
    print(f"Device: {device}")
    print(f"Workloads: {run_workloads}")
    if sweep_mode:
        print(f"Sweep variant: {args.variant_tag}  model_dir: {args.model_dir}")
    ...
    for wl in run_workloads:
        ...
        if result:
            s36_results[wl] = result

    # Save S36 results
    if sweep_mode:
        out_dir = Path("outputs/phase4/timegan_s36_tier2_sweep")
        out_dir.mkdir(parents=True, exist_ok=True)
        out_file = out_dir / f"eval_{args.workload}_{args.variant_tag}.json"
    else:
        out_dir = Path("outputs/phase4/timegan_s36_tier2")
        out_dir.mkdir(parents=True, exist_ok=True)
        out_file = out_dir / "s36_eval_results.json"
    ...
    with open(out_file, "w") as f:
        json.dump({
            ...
            "workloads": s36_results,
            "summary": {
                "mean_vr_smooth": float(np.mean([r["vr_smooth"] for r in s36_results.values()])),
                "pass_count": int(sum(1 for r in s36_results.values() if r["pass_smooth"])),
                "total": int(len(s36_results)),
            }
        }, f, indent=2)
    
    # Comparison summary
    print("\n" + "="*80)
    print("S36 vs S34 COMPARISON (SMOOTHED VR)")
    print("="*80)
    
    if s34_results:
        print(f"\n{'Workload':<15} {'S36 VR':<10} {'S34 VR':<10} {'Change':<12} {'Direction'}")
        print("-"*80)
        ...
        for wl in run_workloads:
            if wl not in s36_results:
                continue

            s36_vr = s36_results[wl]["vr_smooth"]
            s34_vr = s34_results["workloads"].get(wl, {}).get("vr_smooth", 0)
            
            # Calculate distance from ideal VR=1.0
            s36_dist = abs(s36_vr - 1.0)
            s34_dist = abs(s34_vr - 1.0)
            
            change = s36_dist - s34_dist  # Negative = improvement (closer to 1.0)
            ...
            status = "PASS" if s36_results[wl]["pass_smooth"] else "FAIL"
            
            print(f"{wl:<15} {s36_vr:<10.3f} {s34_vr:<10.3f} {change:+.3f} ({direction:<8}) {status}")
        
        # Mean comparison
        s36_mean = float(np.mean([r["vr_smooth"] for r in s36_results.values()]))
        s34_mean = s34_results.get("summary", {}).get("mean_vr_smooth", 0)
        
        s36_mean_dist = abs(s36_mean - 1.0)
        s34_mean_dist = abs(s34_mean - 1.0)
        mean_change = s36_mean_dist - s34_mean_dist
        
        print("-"*80)
        print(f"{'MEAN':<15} {s36_mean:<10.3f} {s34_mean:<10.3f} {mean_change:+.3f}")
        ...
        if mean_change < -0.05:
            print("\nSIGNIFICANT IMPROVEMENT - Spectral normalization helped!")
            print("Decision: Proceed to S37 (add label smoothing)")
        elif mean_change < 0:
            print("\nMINOR IMPROVEMENT - Spectral normalization helped slightly")
            print("Decision: Proceed to S37 (add label smoothing)")
        elif mean_change < 0.05:
            print("\nNO SIGNIFICANT CHANGE - Spectral normalization had minimal effect")
            print("Decision: Analyze results, consider proceeding to S37")
        else:
            print("\nDEGRADATION - Spectral normalization hurt performance")
            print("Decision: Investigate why before continuing")
        
        print(f"\nMean VR distance from 1.0:")
        print(f"  S34: {s34_mean_dist:.3f}")
        print(f"  S36: {s36_mean_dist:.3f}")
        print(f"  Change: {mean_change:+.3f} ({'closer to 1.0' if mean_change < 0 else 'further from 1.0'})")
    
    else:
        # No S34 baseline
        print(f"\n{'Workload':<15} {'S36 VR':<10} {'Status'}")
        print("-"*80)
        for wl, res in s36_results.items():
            status = "PASS" if res["pass_smooth"] else "FAIL"
            print(f"{wl:<15} {res['vr_smooth']:<10.3f} {status}")
        
        s36_mean = float(np.mean([r["vr_smooth"] for r in s36_results.values()]))
        print("-"*80)
        print(f"{'MEAN':<15} {s36_mean:<10.3f}")
        
        print("\nNote: Run eval_s34.py to generate baseline for comparison")
    
    # Final summary
    print("\n" + "="*80)
    print("RESULTS SAVED")
    print("="*80)
    print(f"S36 results: {out_file}")
    if s34_results:
        print(f"S34 baseline: outputs/phase4/timegan_s34/s34_eval_results.json")
    print("\n" + "="*80)
