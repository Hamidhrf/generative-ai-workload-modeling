"""
S37 Evaluation - Compare S37 vs S36
====================================
Evaluate S37 models (discriminator dropout) and compare against S36 baseline.

ABLATION STUDY: Does discriminator dropout improve VR?
- S36 baseline: Mean VR = 1.116 (spectral normalization)
- S37: S36 + Dropout (p=0.3) in discriminator LSTM

Usage:
    python eval_s37.py
    python eval_s37.py --workloads bert gpt2
    python eval_s37.py --window_size 3  # Less aggressive smoothing
"""
...
def evaluate_workload(workload_name, device, window_size=5, n_gen=5):
    ...
    # Load S37 model
    model_paths = {
        "bert": "models/phase4/timegan_s37/s37_bert_vr05_fm15/generator.pt",
        "gpt2": "models/phase4/timegan_s37/s37_gpt2_vr03_fm20/generator.pt",
        "resnet152": "models/phase4/timegan_s37/s37_resnet152_vr04_fm10/generator.pt",
        "whisper": "models/phase4/timegan_s37/s37_whisper_vr05_fm05/generator.pt",
        "yolo": "models/phase4/timegan_s37/s37_yolo_vr03_fm12/generator.pt",
    }
    
    model_path = Path(model_paths[workload_name])
    if not model_path.exists():
        print(f"ERROR: No S37 model found for {workload_name}")
...
def main():
    parser = argparse.ArgumentParser(description="S37 Evaluation - Compare vs S36")
    ...
    print("="*80)
    print("S37 EVALUATION - ABLATION STUDY: DISCRIMINATOR DROPOUT")
    print("="*80)
    print(f"Device: {device}")
    print(f"Workloads: {args.workloads}")
    print(f"Window size: {args.window_size}")
    print(f"Dataset: Original 176 traces (NO augmentation)")
    print(f"Architecture: S36 + Dropout (p=0.3) in discriminator")
    print("="*80)
    ...
    with open(out_dir / "s37_eval_results.json", "w") as f:
        json.dump({
            ...
            "workloads": s37_results,
            "summary": {
                "mean_vr_smooth": float(np.mean([r["vr_smooth"] for r in s37_results.values()])),
                "pass_count": int(sum(1 for r in s37_results.values() if r["pass_smooth"])),
                "total": int(len(s37_results)),
            }
        }, f, indent=2)
    
    # Comparison summary
    print("\n" + "="*80)
    print("S37 vs S36 COMPARISON (SMOOTHED VR)")
    print("="*80)
    ...
    if s36_results:
        ...
        for wl in args.workloads:
            if wl not in s37_results:
                continue
            
            s37_vr = s37_results[wl]["vr_smooth"]
            ...
            status = "PASS" if s37_results[wl]["pass_smooth"] else "FAIL"
            
            print(f"{wl:<15} {s37_vr:<10.3f} {s36_vr:<10.3f} {change:+.3f} ({direction:<8}) {status}")
        
        # Mean comparison
        s37_mean = float(np.mean([r["vr_smooth"] for r in s37_results.values()]))
        ...
        if mean_change < -0.05:
            print("\nSIGNIFICANT IMPROVEMENT - Discriminator dropout helped!")
            print("Decision: Proceed to S38 (try next improvement)")
        elif mean_change < 0:
            print("\nMINOR IMPROVEMENT - Discriminator dropout helped slightly")
            print("Decision: Proceed to S38 (try next improvement)")
        elif mean_change < 0.05:
            print("\nNO SIGNIFICANT CHANGE - Discriminator dropout had minimal effect")
            print("Decision: Analyze results, consider proceeding to S38")
        else:
            print("\nDEGRADATION - Discriminator dropout hurt performance")
        ...
        print("-"*80)
        for wl, res in s37_results.items():
