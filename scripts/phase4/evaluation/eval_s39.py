"""
S39 Evaluation - Compare S39 vs S36
====================================
Evaluate S39 models (targeted variance tuning) and compare against S36 baseline.

ABLATION STUDY: Does targeted hyperparameter tuning improve VR?
- S36 baseline: Mean VR = 1.116 (GPT2=1.137, Whisper=1.397)
- S39: S36 + Increased lambda_var_reg for GPT2 and Whisper

TARGETED CHANGES:
- GPT2: lambda_var_reg 0.3 -> 0.6
- Whisper: lambda_var_reg 0.5 -> 0.8
- BERT, ResNet152, YOLO: Use S36 models (unchanged)

Usage:
    python eval_s39.py
    python eval_s39.py --workloads gpt2 whisper  # Only changed workloads
    python eval_s39.py --window_size 3  # Less aggressive smoothing
"""
...
def evaluate_workload(workload_name, device, window_size=5, n_gen=5):
    ...
    # Load S39 models (GPT2/Whisper retuned, others from S36)
    model_paths = {
        "bert": "models/phase4/timegan_s36/s36_bert_vr05_fm15/generator.pt",  # Use S36
        "gpt2": "models/phase4/timegan_s39/s39_gpt2_vr06_fm20/generator.pt",  # S39 tuned
        "resnet152": "models/phase4/timegan_s36/s36_resnet152_vr04_fm10/generator.pt",  # Use S36
        "whisper": "models/phase4/timegan_s39/s39_whisper_vr08_fm05/generator.pt",  # S39 tuned
        "yolo": "models/phase4/timegan_s36/s36_yolo_vr03_fm12/generator.pt",  # Use S36
    }
    
    model_path = Path(model_paths[workload_name])
    if not model_path.exists():
        print(f"ERROR: No S39/S36 model found for {workload_name}")
...
def main():
    ...
    print(f"Changes: GPT2 lambda_var 0.3->0.6, Whisper 0.5->0.8")
    print("="*80)
    ...
    for wl in args.workloads:
        ...
        if result:
            s39_results[wl] = result
    
    # Save S39 results
    out_dir = Path("outputs/phase4/timegan_s39")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    with open(out_dir / "s39_eval_results.json", "w") as f:
        json.dump({
            "window_size": args.window_size,
            "smoothing_enabled": True,
            "dataset_size": "176 traces (original, no augmentation)",
            "improvement": "targeted_variance_tuning",
            "changes": "GPT2 lambda_var 0.3->0.6, Whisper 0.5->0.8",
            "workloads": s39_results,
            "summary": {
                "mean_vr_smooth": float(np.mean([r["vr_smooth"] for r in s39_results.values()])),
                "pass_count": int(sum(1 for r in s39_results.values() if r["pass_smooth"])),
                "total": int(len(s39_results)),
            }
        }, f, indent=2)
    
    # Comparison summary
    print("\n" + "="*80)
    print("S39 vs S36 COMPARISON (SMOOTHED VR)")
    print("="*80)
    
    if s36_results:
        print(f"\n{'Workload':<15} {'S39 VR':<10} {'S36 VR':<10} {'Change':<12} {'Direction'}")
        print("-"*80)
        ...
        for wl in args.workloads:
            if wl not in s39_results:
                continue
            
            s39_vr = s39_results[wl]["vr_smooth"]
            s36_vr = s36_results["workloads"].get(wl, {}).get("vr_smooth", 0)
            
            # Calculate distance from ideal VR=1.0
            s39_dist = abs(s39_vr - 1.0)
            s36_dist = abs(s36_vr - 1.0)
            
            change = s39_dist - s36_dist  # Negative = improvement (closer to 1.0)
            ...
            status = "PASS" if s39_results[wl]["pass_smooth"] else "FAIL"
            
            print(f"{wl:<15} {s39_vr:<10.3f} {s36_vr:<10.3f} {change:+.3f} ({direction:<8}) {status}")
        
        # Mean comparison
        s39_mean = float(np.mean([r["vr_smooth"] for r in s39_results.values()]))
        s36_mean = s36_results.get("summary", {}).get("mean_vr_smooth", 0)
        
        s39_mean_dist = abs(s39_mean - 1.0)
        s36_mean_dist = abs(s36_mean - 1.0)
        mean_change = s39_mean_dist - s36_mean_dist
        
        print("-"*80)
        print(f"{'MEAN':<15} {s39_mean:<10.3f} {s36_mean:<10.3f} {mean_change:+.3f}")
        ...
        print(f"\nWorkload outcomes:")
        print(f"  Improved: {improvements}/{len(args.workloads)}")
        print(f"  Degraded: {degradations}/{len(args.workloads)}")
        print(f"  Neutral: {len(args.workloads) - improvements - degradations}/{len(args.workloads)}")
        
        if mean_change < -0.05:
            print("\nSIGNIFICANT IMPROVEMENT - Targeted variance tuning worked!")
            print("Decision: Use S39 as final model")
        elif mean_change < 0:
            print("\nMINOR IMPROVEMENT - Targeted variance tuning helped slightly")
            print("Decision: Use S39 as final model")
        elif mean_change < 0.05:
            print("\nNO SIGNIFICANT CHANGE - Targeted tuning had minimal effect")
            print("Decision: Analyze results carefully")
        else:
            print("\nDEGRADATION - Targeted tuning hurt performance")
            print("Decision: Stick with S36")
        
        print(f"\nMean VR distance from 1.0:")
        print(f"  S36: {s36_mean_dist:.3f}")
        print(f"  S39: {s39_mean_dist:.3f}")
        print(f"  Change: {mean_change:+.3f} ({'closer to 1.0' if mean_change < 0 else 'further from 1.0'})")
    
    else:
        # No S36 baseline
        print(f"\n{'Workload':<15} {'S39 VR':<10} {'Status'}")
        print("-"*80)
        for wl, res in s39_results.items():
    ...
    print("RESULTS SAVED")
    print("="*80)
    print(f"S39 results: {out_dir / 's39_eval_results.json'}")
    if s36_results:
