#!/usr/bin/env python3
"""
Comprehensive S27 vs S33 vs S34 Evaluation
===========================================
...
Usage:
  python comprehensive_eval_s27_s33_s34.py
  python comprehensive_eval_s27_s33_s34.py --workloads bert gpt2
"""
...
# Config
WORKLOADS_GPU_BOUND = ['bert', 'gpt2', 'resnet152', 'yolo']  # S27 + S33
WORKLOADS_ALL = ['bert', 'gpt2', 'resnet152', 'whisper', 'yolo']  # S34
...
# S34: All 5 workloads with same 7 metrics
S34_TRAINED = {
    'bert':      [0, 1, 2, 3, 4, 5, 8],
    'gpt2':      [0, 1, 2, 3, 4, 5, 8],
    'resnet152': [0, 1, 2, 3, 4, 5, 8],
    'whisper':   [0, 1, 2, 3, 4, 5, 8],
    'yolo':      [0, 1, 2, 3, 4, 5, 8],
}
...
def load_generator(workload, model_name, device='cuda'):
    ...
    if model_name == 's27':
        ...
        ckpt_path = Path(s34_paths[workload])
        trained_indices = S34_TRAINED[workload]
...
def generate_traces(gen, workload, replica_count, n_samples, trained_indices, device='cuda'):
    ...
    for _ in range(n_samples):
        ...
        # Build full trace (for trained metrics only)
...
def get_real_traces(workload, replica_count):
    ...
    # Get trained metric indices (use S34 for consistency - same 7 metrics everywhere)
    trained_indices = S34_TRAINED[workload]
...
def main():
    parser = argparse.ArgumentParser(description="Comprehensive S27 vs S33 vs S34 Evaluation")
    parser.add_argument("--workloads", nargs='+', type=str, default=WORKLOADS_ALL,
                        help="Workloads to evaluate (default: all 5)")
    args = parser.parse_args()
    
    print("="*100)
    print("COMPREHENSIVE S27 vs S33 vs S34 EVALUATION")
    print("="*100)
    ...
    for workload in args.workloads:
        ...
        # Evaluate S27 (smoothed) - if workload is GPU-bound
        if workload in WORKLOADS_GPU_BOUND:
            print("\n[1/3] S27 (smoothed)")
            try:
        ...
        # Evaluate S34 (smoothed) - all workloads
        print(f"\n[3/3] S34 (smoothed)")
        try:
            result_s34 = evaluate_model(workload, 's34', apply_smoothing=True)
            all_results.append(result_s34)
        except Exception as e:
            print(f"  ERROR: {e}")
    ...
    output_path = output_dir / 'comprehensive_results_s27_s33_s34.json'
