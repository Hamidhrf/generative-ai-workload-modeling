"""
S35 Evaluation - Compare S35 vs S34
====================================
Evaluate S35 models (4x augmented data) and compare against S34 baseline.

ABLATION STUDY: Does 4x data augmentation improve VR?
- S34 baseline: 176 traces, mean VR = ?
- S35: 704 traces (4x augmented), mean VR = ?

Usage:
    python eval_s35.py
    python eval_s35.py --workloads bert gpt2
    python eval_s35.py --window_size 3  # Less aggressive smoothing
"""
...
# Paths
DATA_COMBINED = Path("data/processed/phase4/unified/combined_dataset_s35_augmented.npz")
NORM_COMBINED = Path("data/processed/phase4/unified/combined_normalization_s35_augmented.json")
...
def evaluate_workload(workload_name, device, window_size=5, n_gen=5):
    ...
    # Load data (using augmented dataset for validation set)
    ...
    # Get kept metrics (same 7 for all S35 workloads)
    ...
    if not model_path.exists():
        print(f"ERROR: No S35 model found for {workload_name}")
...
def main():
    parser = argparse.ArgumentParser(description="S35 Evaluation - Compare vs S34")
    ...
    print("="*80)
    print("S35 EVALUATION - ABLATION STUDY: DATA AUGMENTATION")
    print("="*80)
    print(f"Device: {device}")
    print(f"Workloads: {args.workloads}")
    print(f"Window size: {args.window_size}")
    print(f"Dataset: AUGMENTED (704 traces, 4x original)")
    print(f"Architecture: IDENTICAL to S34")
    print("="*80)
    ...
    for wl in args.workloads:
        ...
        if result:
            s35_results[wl] = result
    
    # Save S35 results
    out_dir = Path("outputs/phase4/timegan_s35")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    with open(out_dir / "s35_eval_results.json", "w") as f:
        json.dump({
            "window_size": args.window_size,
            "smoothing_enabled": True,
            "dataset_size": "704 traces (4x augmented)",
            "workloads": s35_results,
            "summary": {
                "mean_vr_smooth": float(np.mean([r["vr_smooth"] for r in s35_results.values()])),
                "pass_count": int(sum(1 for r in s35_results.values() if r["pass_smooth"])),
                "total": int(len(s35_results)),
            }
        }, f, indent=2)
    
    # Comparison summary
    print("\n" + "="*80)
    print("S35 vs S34 COMPARISON (SMOOTHED VR)")
    print("="*80)
    
    if s34_results:
        print(f"\n{'Workload':<15} {'S35 VR':<10} {'S34 VR':<10} {'Change':<12} {'Status'}")
        print("-"*80)
        
        for wl in args.workloads:
            if wl not in s35_results:
                continue
            
            s35_vr = s35_results[wl]["vr_smooth"]
            s34_vr = s34_results["workloads"].get(wl, {}).get("vr_smooth", 0)
            
            change = s35_vr - s34_vr
            direction = "IMPROVED" if change > 0 else "DEGRADED"
            status = "PASS" if s35_results[wl]["pass_smooth"] else "FAIL"
            
            print(f"{wl:<15} {s35_vr:<10.3f} {s34_vr:<10.3f} {change:+.3f} ({direction:<8}) {status}")
        
        # Mean comparison
        s35_mean = float(np.mean([r["vr_smooth"] for r in s35_results.values()]))
        s34_mean = s34_results.get("summary", {}).get("mean_vr_smooth", 0)
        mean_change = s35_mean - s34_mean
        
        print("-"*80)
        print(f"{'MEAN':<15} {s35_mean:<10.3f} {s34_mean:<10.3f} {mean_change:+.3f}")
        ...
        if mean_change > 0.05:
            print("SIGNIFICANT IMPROVEMENT - Data augmentation helped!")
            print("Decision: Proceed to S36 (add spectral normalization)")
        elif mean_change > 0:
            print("MINOR IMPROVEMENT - Data augmentation helped slightly")
            print("Decision: Proceed to S36 (add spectral normalization)")
        elif mean_change > -0.05:
            print("NO SIGNIFICANT CHANGE - Data augmentation had minimal effect")
            print("Decision: Analyze results, may still proceed to S36")
        else:
            print("DEGRADATION - Data augmentation hurt performance")
            print("Decision: Investigate why augmentation failed before continuing")
        
        print(f"\nMean VR change: {mean_change:+.3f}")
        print(f"Target: VR closer to 1.0 (currently: S35={s35_mean:.3f}, S34={s34_mean:.3f})")
    
    else:
        # No S34 baseline
        print(f"\n{'Workload':<15} {'S35 VR':<10} {'Status'}")
        print("-"*80)
        for wl, res in s35_results.items():
            status = "PASS" if res["pass_smooth"] else "FAIL"
            print(f"{wl:<15} {res['vr_smooth']:<10.3f} {status}")
        
        s35_mean = float(np.mean([r["vr_smooth"] for r in s35_results.values()]))
        print("-"*80)
        print(f"{'MEAN':<15} {s35_mean:<10.3f}")
    ...
    print("RESULTS SAVED")
    print("="*80)
    print(f"S35 results: {out_dir / 's35_eval_results.json'}")
    if s34_results:
