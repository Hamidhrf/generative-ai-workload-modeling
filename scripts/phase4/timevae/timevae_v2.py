#!/usr/bin/env python3
"""
Phase 4 - TimeVAE v2: Two targeted fixes over v1
Master Thesis - Generative AI Workload Modeling
Fachhochschule Dortmund

CHANGES FROM v1
===============
v1 mean VR = 0.6128, LSTM baseline = 0.6124. No improvement.
Two root causes identified:

FIX 1: phase_embed_dim 8 -> 4
  v1 BERT jump_ratio=12x, Whisper=11x. The decoder learned to fire hard at
  every phase boundary, producing oversharp transitions that distort all
  other metrics. Same problem appeared in LSTM v2 (phase_embed_dim=8) and
  was fixed by reducing to 4. Applying the same fix here.

FIX 2: scheduled sampling for autoregressive decoder
  During training, teacher forcing feeds the REAL previous output as decoder
  input. At generation, the decoder uses its OWN previous output. If the model
  never learned to produce spikes autoregressively (because it always got the
  real spike fed to it), spikes cannot appear at generation time.

  Scheduled sampling: with probability (1 - teacher_forcing_ratio), use the
  model's own previous output instead of the real value as decoder input.
  teacher_forcing_ratio decays from 1.0 -> tf_min over ss_anneal_epochs.

  This forces the model to learn to generate spikes from its own outputs
  rather than relying on the real trace signal.
...
OUTPUT_DIR   = Path("outputs/phase4/timevae_v2")
MODEL_DIR    = Path("models/phase4/timevae_v2")
...
TRAIN_CFG = {
    ...
    "beta_max":          0.5,
    "warmup_epochs":     50,
    # Fix 1: phase_embed_dim reduced 8->4 to prevent phase boundary explosion
    "phase_embed_dim":   4,
    # Fix 2: scheduled sampling - teacher forcing ratio anneals 1.0->tf_min
    # over ss_anneal_epochs, forcing decoder to generate spikes autoregressively
    "tf_ratio_start":    1.0,
    "tf_ratio_min":      0.3,
    "ss_anneal_epochs":  150,
}
...
class TimeVAEDecoder(nn.Module):
    ...
    def forward_teacher(self, z, r_norm, phase_seq_batch, x_real, tf_ratio=1.0):
        """
        Forward pass with optional scheduled sampling.

        tf_ratio: teacher forcing ratio in [0,1].
          1.0 = full teacher forcing (v1 behaviour): decoder always sees real x_{t-1}.
          0.0 = no teacher forcing: decoder always sees its own previous output.
          0.3 = mixed: 30% of timesteps use real input, 70% use own output.

        Scheduled sampling decays tf_ratio from 1.0 toward tf_min over training.
        This forces the model to learn to generate spikes autoregressively rather
        than relying on the real spike being fed as input.
        """
        B, T, M = x_real.shape
        ...
        x_t  = torch.zeros(B, M, device=x_real.device)   # start token
        outs = []

        for t in range(T):
            r_e  = r_embed                                   # (B, r_embed_dim)
            ph_t = ph_emb[:, t, :]                          # (B, phase_embed_dim)
            inp  = torch.cat([x_t, r_e, ph_t], dim=1)      # (B, M+r+ph)
            inp  = inp.unsqueeze(1)                          # (B, 1, ...)
            out_h, (h, c) = self.lstm(inp, (h, c))
            pred = torch.sigmoid(self.fc_out(out_h.squeeze(1)))  # (B, M)
            outs.append(pred)

            # Scheduled sampling: choose next input
            if t < T - 1:
                if tf_ratio >= 1.0:
                    x_t = x_real[:, t, :]          # always use real
                elif tf_ratio <= 0.0:
                    x_t = pred.detach()            # always use own output
                else:
                    # Sample per-batch: use real or own output with probability tf_ratio
                    use_real = torch.rand(B, device=x_real.device) < tf_ratio
                    use_real = use_real.unsqueeze(1)  # (B, 1) for broadcast
                    x_t = torch.where(use_real, x_real[:, t, :], pred.detach())

        return torch.stack(outs, dim=1)   # (B, T, M)
...
class TimeVAE(nn.Module):
    ...
    def forward(self, x, r_norm, phase_seq_batch, tf_ratio=1.0):
        mu, logvar = self.encoder(x)
        z          = self.reparameterize(mu, logvar)
        recon      = self.decoder.forward_teacher(z, r_norm, phase_seq_batch, x, tf_ratio)
...
def train(model, train_dl, val_dl, cfg, device, metric_weights=None):
    ...
    for epoch in range(1, cfg["epochs"] + 1):
        ...
        # Scheduled sampling: tf_ratio decays from tf_ratio_start -> tf_ratio_min
        ss_progress = min(1.0, epoch / max(cfg["ss_anneal_epochs"], 1))
        tf_ratio = (cfg["tf_ratio_start"]
                    - (cfg["tf_ratio_start"] - cfg["tf_ratio_min"]) * ss_progress)
        ...
        for x, r_norm, phase_t in train_dl:
            ...
            recon, mu, logvar = model(x, r_norm, phase_t, tf_ratio)
        ...
        with torch.no_grad():
            for x, r_norm, phase_t in val_dl:
                x, r_norm = x.to(device), r_norm.to(device)
                phase_t   = phase_t.to(device)
                # Validate without teacher forcing to measure generation quality
                recon, mu, logvar = model(x, r_norm, phase_t, tf_ratio=0.0)
                loss, _, _ = vae_loss(recon, x, mu, logvar, beta, metric_weights)
        ...
        if epoch % 25 == 0 or epoch == 1:
            print(f"    epoch {epoch:4d}  val_loss={val_loss:.5f}  "
                  f"beta={beta:.3f}  tf_ratio={tf_ratio:.3f}")
        ...
        if val_loss < best_val - cfg["min_delta"]:
            ...
            if patience_c >= cfg["patience"]:
                print(f"    early stop at epoch {epoch}  tf_ratio={tf_ratio:.3f}")
                break
...
def plot_comparison(workload, real_orig, syn_orig,
    ...
    path = save_dir / f"{workload}_timevae_v2.png"
...
def main():
    ...
    for workload in args.workloads:
        ...
        if workload in lstm_vr:
            ...
            print(f"  LSTM baseline VR:  {lstm_vr[workload]:.4f}  (delta: {delta_lstm:+.4f})")
            if workload in vae_v1_vr:
                print(f"  TimeVAE v1 VR:     {vae_v1_vr[workload]:.4f}  (delta: {delta_v1:+.4f})")
    ...
    # ------------------------------------------------------------------
    print(f"\n{'='*65}")
    print("TIMEVAE v2 SUMMARY  (phase_embed_dim=4, scheduled sampling)")
    print(f"{'='*65}")
    col = 10
    header = (f"{'Workload':<12}  {'VAE-v2':>{col}}  {'VAE-v1':>{col}}"
              f"  {'LSTM':>{col}}  {'vs v1':>{col}}  {'vs LSTM':>{col}}")
    print(header)
    print("-" * len(header))
    for r in all_results:
        v1  = vae_v1_vr.get(r["workload"], float("nan"))
        lv  = lstm_vr.get(r["workload"],   float("nan"))
        dv1 = r["var_ratio_mean"] - v1  if not np.isnan(v1) else float("nan")
        dlv = r["var_ratio_mean"] - lv  if not np.isnan(lv) else float("nan")
    ...
    print(f"  {'MEAN':<12}  {np.mean(vr_vals):>{col}.4f}"
          f"  {np.mean(list(vae_v1_vr.values())):>{col}.4f}"
          f"  {np.mean(list(lstm_vr.values())):>{col}.4f}")
