...
- 2026-08-18T07:24:33Z Tier 3 whisper r=1: 22 CSV files, mean rows=715
- 2026-08-18T08:30:32Z Tier 3 whisper r=2: 22 CSV files, mean rows=942
- 2026-08-18T09:36:31Z Tier 3 whisper r=3: 22 CSV files, mean rows=1170
- 2026-08-18T10:42:31Z Tier 3 whisper r=4: 22 CSV files, mean rows=1397
...
- 2026-08-18T16:12:34Z Tier 3 whisper r=9: 22 CSV files, mean rows=2518
- 2026-08-18T17:18:35Z Tier 3 whisper r=10: 22 CSV files, mean rows=2737

## Tier 2 Step 2: sanity + per-slice vs averaged diagnostic — 2026-08-20

3-column raw per-metric mean audit (BERT, A16/Tier2/Tier3, denormalized): gpu_memory_total came in at 2720.31 MiB (A16), 2751.75 MiB (Tier2), 17330.64 MiB (Tier3) — none matched the pre-registered physical expectation (A16 ~15000-class, Tier2 ~11007 constant per-slice, Tier3 ~95000-class whole-GPU). Cause: gpu_memory_total is one of the aggregated GPU metrics divided by replica_count in create_pod_traces (inherited unchanged from preprocess_tier3.py, applied identically across all three tiers' pipelines), so the reported mean is total-VRAM/replica_count averaged over r=1..10 (A16, Tier3) or r=1..7 (Tier2), not raw hardware capacity — a preprocessing artifact affecting all three tiers equally, not a Tier2-specific issue. gpu_utilization came in at 2.6959 (A16), 1.8374 (Tier2), 0.8952 (Tier3): the prediction that Tier2 would read dramatically lower than Tier3 did not hold — Tier2 is roughly 2x Tier3 for BERT, the opposite direction. pod_psi_cpu was 0.0001 across all three tiers for BERT (no CPU contention signal, consistent with BERT being GPU-bound); the whisper-specific CPU-contention prediction could not be checked against this table since it is BERT-only by spec.

Cross-pod spread audit, gpu_utilization at r=7, all 5 workloads (max-min spread / stdev of per-pod means): bert 0.0392/0.0139, gpt2 0.5766/0.2322, resnet152 0.0678/0.0277, whisper 0.6147/0.1854, yolo 0.0283/0.0119. Against the stated tight-cluster bar (spread<0.5, stdev<0.2): bert, resnet152, yolo meet both. gpt2 fails both (widest spread and stdev of all 5 workloads) — not the workload predicted to depart from tight clustering. whisper fails only the spread threshold (0.6147>0.5) but meets the stdev threshold (0.1854<0.2), a partial rather than clean departure.

Per-slice-vs-averaged diagnostic, gpu_utilization at r=7 (mean_cross_pod_var_real, var_of_pod_means_real, fraction_of_total_variance): bert 0.041933/0.000193/0.000143, gpt2 2.183969/0.053895/0.001108, resnet152 0.035016/0.000767/0.000485, whisper 0.298341/0.034387/0.010106, yolo 0.000666/0.000141/0.000406. gpt2's mean_cross_pod_var_real (2.184) is ~7.3x whisper's (0.298) and the largest of all 5 workloads, consistent with the spread-table finding that gpt2, not whisper, has the widest per-timestep cross-pod scatter. whisper has the highest fraction_of_total_variance (1.01%) despite a smaller mean_cross_pod_var_real than gpt2, because whisper's overall gpu_utilization variance (the denominator) is smaller. All five fractions are under 1.1% — cross-pod identity structure is a small share of total signal variance for every workload at r=7, even the widest-spread ones.
